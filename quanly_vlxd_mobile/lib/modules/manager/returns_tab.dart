...
class _ReturnsTabState extends State<ReturnsTab>
  ...
  Future<void> _fetchReturnsData() async {
    ...
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('Lỗi tải dữ liệu Đổi/Trả: $e'),
    ...
    return source.where((item) {
      ...
      final ma =
          (item['maDT'] ??
              ...
              .toString()
              .toLowerCase();
      final ten = (item['tenKhachHang'] ?? item['tenNhaCungCap'] ?? '')
          .toString()
          .toLowerCase();
      final matchQuery =
      ...
      final matchStatus =
          ...
          st.toLowerCase().contains(_selectedStatus.toLowerCase());
      ...
      builder: (context) {
        return AlertDialog(
                  ...
                  Card(
                    ...
                    child: Padding(
                      ...
                      child: Column(
                        ...
                        children: [
                          Text(
                            '${isCustomer ? "Khách hàng" : "Nhà cung cấp"}: $tenDoiTac',
                            style: const TextStyle(
                              fontWeight: FontWeight.bold,
                              fontSize: 15,
                            ),
                          ...
                          Text(
                            'Ngày yêu cầu: ${ngay != null ? DateFormat('dd/MM/yyyy HH:mm').format(DateTime.parse(ngay)) : 'N/A'}',
                          ),
                          const SizedBox(height: 8),
                          ...
                          if (isCustomer) ...[
                            const SizedBox(height: 8),
                            Text(
                              'Loại: ${item['loai'] ?? 'N/A'} | Lỗi do: ${item['loiDo'] ?? 'N/A'}',
                            ),
                          ],
                      ...
                      rows: chiTiet.map((ct) {
                        return DataRow(
                          cells: [
                            ...
                            DataCell(
                              Text('${ct['soLuong'] ?? ct['soLuongTra'] ?? 0}'),
                            ),
                            DataCell(
                              Text(
                                NumberFormat.currency(
          ...
          actions: [
            ...
            if (currentStatus.contains('Chờ') ||
                currentStatus.contains('Đề Xuất')) ...[
              ElevatedButton.icon(
                onPressed: () async {
                  ...
                  try {
                    ...
                    final res = isCustomer
                        ? await _apiService.approveCustomerReturn(id)
                        : await _apiService.approveSupplierReturn(id);
                    if (!mounted) return;
                    ScaffoldMessenger.of(context).showSnackBar(
                      SnackBar(
                        content: Text(
                          res.data['message'] ?? 'Đã duyệt thành công!',
                        ),
                    ...
                    ScaffoldMessenger.of(context).showSnackBar(
                      SnackBar(
                        content: Text('Lỗi duyệt phiếu: $e'),
                        backgroundColor: Colors.red,
                      ),
                    );
            ...
            if (currentStatus.contains('Duyệt') ||
                currentStatus.contains('Chờ Hàng Về') ||
                currentStatus.contains('Đang đổi trả')) ...[
              ElevatedButton.icon(
                onPressed: () async {
                  Navigator.pop(context);
                  setState(() => _isLoading = true);
                  try {
                    final id = item['maPhieuDT'] ?? item['maPhieuTra'];
                    final res = isCustomer
                        ? await _apiService.receiveCustomerReturn(id)
                        : await _apiService.receiveSupplierReturn(id);
                    if (!mounted) return;
                    ScaffoldMessenger.of(context).showSnackBar(
                      SnackBar(
                        content: Text(
                    ...
                    ScaffoldMessenger.of(context).showSnackBar(
                      SnackBar(
                        content: Text('Lỗi xác nhận nhập kho: $e'),
                        backgroundColor: Colors.red,
                      ),
                    );
                    setState(() => _isLoading = false);
        ...
        return StatefulBuilder(
          builder: (context, setPopupState) {
            return AlertDialog(
              ...
              title: const Text(
                'Tạo Đề Nghị Đổi/Trả Khách Hàng',
                style: TextStyle(fontWeight: FontWeight.bold),
              ),
              content: SizedBox(
                width: double.maxFinite,
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      DropdownButtonFormField<int>(
                        ...
                        items: orders.map<DropdownMenuItem<int>>((o) {
                          return DropdownMenuItem<int>(
                            ...
                            child: Text(
                              '${o['maHD']} - ${o['tenKH'] ?? o['tenKhachHang'] ?? "Khách hàng"}',
                            ),
                          );
                        }).toList(),
                        onChanged: (val) async {
                          ...
                          if (val != null) {
                            try {
                              ...
                              final resDetail = await _apiService
                                  .getOrderDetail(val);
                              if (resDetail.statusCode == 200 &&
                                  resDetail.data != null) {
                                setPopupState(() {
                                  candidateItems =
                                      resDetail.data['cthDs'] ??
                                      resDetail.data['chiTiet'] ??
                                      [];
                                  returnItems = candidateItems
                                      .map(
                                        (c) => {
                      ...
                      if (returnItems.isEmpty)
                        const Text(
                          'Vui lòng chọn Hóa đơn để hiển thị sản phẩm',
                          style: TextStyle(
                            color: Colors.grey,
                          ...
                          children: returnItems.map((item) {
                            return CheckboxListTile(
                              ...
                              subtitle: Row(
                                children: [
                                  Text(
                                    'Đơn giá: ${NumberFormat.currency(locale: 'vi_VN', symbol: 'đ').format(item['donGia'])}',
                                  ),
                                  ...
                                  if (item['selected'])
                                    Expanded(
                                      child: TextFormField(
                                        ...
                                        decoration: InputDecoration(
                                          labelText:
                                              'Số lượng (tối đa ${item['maxSoLuong']})',
                                          isDense: true,
                                        ),
                                        onChanged: (val) {
                                          int sl = int.tryParse(val) ?? 1;
                                          if (sl > item['maxSoLuong'])
                                            sl = item['maxSoLuong'];
                                          item['soLuong'] = sl;
                                        },
                                      ),
                              ...
                              onChanged: (bool? val) => setPopupState(
                ...
                ElevatedButton.icon(
                  onPressed:
                      selectedOrderId == null ||
                          !returnItems.any((i) => i['selected'])
                      ? null
                      : () async {
                          Navigator.pop(context);
                          setState(() => _isLoading = true);
                          try {
                            final selectedList = returnItems
                                .where((i) => i['selected'])
                                .toList();
                            final payload = {
                              'maHoaDon': selectedOrderId,
                              'maNhanVien': 1, // Giả định NV 1
                              'lyDo': reasonCtrl.text.isNotEmpty
                                  ? reasonCtrl.text
                                  : 'Yêu cầu từ Mobile',
                              'loiDo': faultBy,
                              'loai': returnType,
                            ...
                            final res = await _apiService.createCustomerReturn(
                              payload,
                            );
                            if (!mounted) return;
                            ScaffoldMessenger.of(context).showSnackBar(
                            ...
                            ScaffoldMessenger.of(context).showSnackBar(
                              SnackBar(
                                content: Text('Lỗi tạo đề nghị: $e'),
                                backgroundColor: Colors.red,
                              ),
                            );
                            setState(() => _isLoading = false);
          ...
          builder: (context, setPopupState) {
            return AlertDialog(
                    ...
                    DropdownButtonFormField<int>(
                      ...
                      items: procurements.map<DropdownMenuItem<int>>((p) {
                        return DropdownMenuItem<int>(
                          value: p['maPhieuNhap'],
                          child: Text(
                            '${p['maPN'] ?? p['maPhieuNhap']} - ${p['tenNhaCungCap'] ?? "NCC"}',
                          ),
                        );
                      }).toList(),
                      onChanged: (val) =>
                          setPopupState(() => selectedProcurementId = val),
                    ),
                    const SizedBox(height: 16),
                    TextField(
                      controller: reasonCtrl,
              ...
              actions: [
                ...
                ElevatedButton.icon(
                  onPressed: selectedProcurementId == null
                      ? null
                      : () async {
                          Navigator.pop(context);
                          setState(() => _isLoading = true);
                          try {
                            final payload = {
                              ...
                              'lyDo': reasonCtrl.text.isNotEmpty
                                  ? reasonCtrl.text
                                  : 'Hoàn trả hàng nhập lỗi/thiếu',
                            };
                            final res = await _apiService.createSupplierReturn(
                              payload,
                            );
                            if (!mounted) return;
                            ScaffoldMessenger.of(context).showSnackBar(
                              SnackBar(
                                content: Text(
                                  res.data['message'] ??
                                      'Tạo đề nghị trả hàng NCC thành công!',
                                ),
                                backgroundColor: Colors.green,
                              ),
                            );
                            ...
                            ScaffoldMessenger.of(context).showSnackBar(
                              SnackBar(
                                content: Text('Lỗi tạo đề nghị trả NCC: $e'),
                                backgroundColor: Colors.red,
                              ),
                            );
                            setState(() => _isLoading = false);
  ...
  Widget build(BuildContext context) {
    ...
    final currentList = _getFilteredList(
      isCustomerTab ? _customerReturns : _supplierReturns,
    );
        ...
        children: [
                  ...
                  Row(
                    children: [
                      ...
                      if (PermissionHelper.canCreate('RETURNS')) ...[
                        const SizedBox(width: 12),
                        ElevatedButton.icon(
                          onPressed: isCustomerTab
                              ? _showAddCustomerReturnPopup
                              : _showAddSupplierReturnPopup,
                          icon: const Icon(Icons.add),
                          label: Text(
                            isCustomerTab ? 'TẠO ĐỀ NGHỊ' : 'TRẢ HÀNG NCC',
          ...
          Expanded(
            child: _isLoading
                ...
                ? const Center(
                    child: Text(
                      'Không tìm thấy phiếu đổi/trả nào phù hợp',
        ...
        child: SingleChildScrollView(
          child: DataTable(
            ...
            rows: list.map((item) {
              ...
              final ten =
                  item['tenKhachHang'] ?? item['tenNhaCungCap'] ?? 'N/A';
              final ngay = item['ngayDT'] ?? item['ngayTra'];
        ...
        return Card(
          ...
          child: ListTile(
            ...
            leading: CircleAvatar(
              ...
              child: Icon(
                isCustomer ? Icons.assignment_return : Icons.outbox,
                color: _getStatusColor(status),
              ),
            ),
            title: Row(
            ...
            subtitle: Column(
              ...
              children: [
                ...
                Text(
                  'Ngày yêu cầu: ${ngay != null ? DateFormat('dd/MM/yyyy').format(DateTime.parse(ngay)) : 'N/A'} | Lý do: ${item['lyDo'] ?? "Không có"}',
                ),
                const SizedBox(height: 8),
                Text(
                  'Tổng tiền hoàn: ${NumberFormat.currency(locale: 'vi_VN', symbol: 'đ').format(item['tongTienHoan'] ?? 0)}',
                  style: TextStyle(
                    fontWeight: FontWeight.bold,
                    color: Colors.red.shade700,
                    fontSize: 15,
