...
class _ReportsTabState extends State<ReportsTab> {
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        ...
        children: [
          _buildSummaryCard(
            'Tổng Doanh Thu',
            _currencyFormat.format(s['totalRevenue'] ?? 0),
            Icons.monetization_on,
            Colors.green,
          ),
          _buildSummaryCard(
            'Tổng Công Nợ',
            _currencyFormat.format(s['totalDebt'] ?? 0),
            Icons.money_off,
            Colors.red,
          ),
          _buildSummaryCard(
            'Giá Trị Kho Hàng',
            _currencyFormat.format(s['inventoryValue'] ?? 0),
            Icons.warehouse,
            Colors.blue,
          ),
          _buildSummaryCard(
          ...
          if (_debtAging != null) ...[
            ...
            _buildDebtAgingRow(
              'Quá hạn < 30 ngày',
              _debtAging!['overdue30'],
              Colors.orange,
            ),
            _buildDebtAgingRow(
              'Quá hạn 30 - 60 ngày',
              _debtAging!['overdue60'],
              Colors.deepOrange,
            ),
            _buildDebtAgingRow(
              'Quá hạn > 60 ngày (Khó đòi)',
              _debtAging!['overdueLong'],
              Colors.red,
            ),
          ],
        ],
  ...
  Widget _buildRevenueProfitTab() {
    if (_revenueProfit.isEmpty)
      return const Center(
        child: Text('Không có dữ liệu doanh thu & lợi nhuận'),
      );
        ...
        return Card(
          child: ListTile(
            ...
            subtitle: Column(
              ...
              children: [
                Text(
                  'Doanh thu: ${_currencyFormat.format(r['revenue'] ?? 0)}',
                  style: const TextStyle(
                    color: Colors.green,
                    fontWeight: FontWeight.bold,
                  ),
                ...
                Text(
                  'Đã thu: ${_currencyFormat.format(r['collected'] ?? 0)} | Số đơn: ${r['orderCount']}',
            ...
            trailing: Column(
              ...
              children: [
                ...
                Text(
                  _currencyFormat.format(r['profit'] ?? 0),
                  style: const TextStyle(
                    color: Colors.blue,
                    fontWeight: FontWeight.bold,
                    fontSize: 14,
      ...
      itemBuilder: (context, index) {
        ...
        return Card(
          child: ListTile(
            ...
            trailing: Text(
              _currencyFormat.format(c['totalSpend'] ?? 0),
              style: const TextStyle(
                color: Colors.green,
                fontWeight: FontWeight.bold,
                fontSize: 15,
  ...
  Widget _buildInventoryAgingTab() {
    if (_inventoryAging.isEmpty)
      return const Center(child: Text('Không có hàng tồn đọng quá 60 ngày'));
    return ListView.builder(
      padding: const EdgeInsets.all(8),
