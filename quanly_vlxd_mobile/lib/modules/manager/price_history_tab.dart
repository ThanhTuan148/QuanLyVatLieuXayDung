...
class _PriceHistoryTabState extends State<PriceHistoryTab>
  ...
  Future<void> _fetchPriceHistoryData() async {
    ...
    try {
      ...
      final resHist = await _apiService.getPriceHistoryFiltered(
        days: _selectedDays,
      );
      final resSumm = await _apiService.getPriceHistorySummary(
        days: _selectedDays,
      );

      if (!mounted) return;
      setState(() {
        if (resOver.statusCode == 200 && resOver.data != null)
          _overviewList = resOver.data is List ? resOver.data : [];
        if (resHist.statusCode == 200 && resHist.data != null)
          _historyList = resHist.data is List ? resHist.data : [];
        if (resSumm.statusCode == 200 && resSumm.data != null)
          _summaryData = resSumm.data;
      });
    } catch (e) {
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('Lỗi tải dữ liệu Lịch sử giá: $e'),
  ...
  List<dynamic> _getFilteredList(List<dynamic> source) {
    return source.where((item) {
      final ma = (item['maSP'] ?? item['maSanPham'] ?? '')
          .toString()
          .toLowerCase();
      final ten = (item['tenSP'] ?? item['tenSanPham'] ?? '')
          .toString()
          .toLowerCase();
      return ma.contains(_searchQuery.toLowerCase()) ||
  ...
  void _showChartDialog(int productId, String productName) async {
    ...
    try {
      final resChart = await _apiService.getPriceHistoryChart(
        productId,
        days: _selectedDays,
      );
      if (!mounted) return;
                      ...
                      children: chartData.map((point) {
                        ...
                        return Card(
                          ...
                          child: ListTile(
                            ...
                            leading: Icon(
                              Icons.timeline,
                              color: isHienTai ? Colors.blue : Colors.green,
                            ),
                            title: Text(
                              'Giá bán: ${NumberFormat.currency(locale: 'vi_VN', symbol: 'đ').format(point['giaBan'] ?? 0)}',
                              style: TextStyle(
                                fontWeight: FontWeight.bold,
                                color: isHienTai ? Colors.blue : Colors.black87,
                              ),
                            ),
                            subtitle: Text(
                              'Ngày: ${point['ngay'] != null ? DateFormat('dd/MM/yyyy HH:mm').format(DateTime.parse(point['ngay'])) : "N/A"} | Nguồn: ${point['nguonThayDoi'] ?? "N/A"}',
                            ),
                            trailing: Text(
                              'Giá nhập: ${NumberFormat.currency(locale: 'vi_VN', symbol: 'đ').format(point['giaNhap'] ?? 0)}',
  ...
  Widget build(BuildContext context) {
    ...
    if (activeIndex == 0)
      currentSource = _overviewList;
    else if (activeIndex == 1)
      currentSource = _historyList;
    else
      currentSource = (_summaryData['chiTiet'] as List<dynamic>?) ?? [];
        ...
        children: [
          ...
          Expanded(
            child: _isLoading
                ...
                ? const Center(
                    child: Text(
                      'Không tìm thấy dữ liệu giá nào phù hợp',
              ...
              rows: list.map((item) {
                ...
                return DataRow(
                  cells: [
                    DataCell(
                      Text(
                        item['maSP'] ?? item['maSanPham'].toString(),
                        style: const TextStyle(
                          fontWeight: FontWeight.bold,
                          color: Colors.blue,
                        ),
                    ...
                    DataCell(
                      Text(
                        NumberFormat.currency(
                          locale: 'vi_VN',
                          symbol: 'đ',
                        ).format(item['giaBanHienTai'] ?? 0),
                        style: const TextStyle(fontWeight: FontWeight.bold),
                      ),
                    ),
                    DataCell(
                      Text(
                        item['giaBanTruoc'] != null
                            ? NumberFormat.currency(
                                locale: 'vi_VN',
                                symbol: 'đ',
                              ).format(item['giaBanTruoc'])
                            : 'N/A',
                      ),
                    ),
                    DataCell(
                      pct == null
                          ...
                          : Container(
                              ...
                              decoration: BoxDecoration(
                                color: isTang
                                    ? Colors.green
                                    : (isGiam ? Colors.red : Colors.orange),
                                borderRadius: BorderRadius.circular(12),
                              ),
                              child: Text(
                                '${pct > 0 ? "+" : ""}$pct%',
              ...
              rows: list.map((item) {
                ...
                return DataRow(
                  cells: [
                    ...
                    DataCell(
                      Text(
                        '${item['giaBanCu'] != null ? NumberFormat.currency(locale: 'vi_VN', symbol: 'đ').format(item['giaBanCu']) : "N/A"} -> ${NumberFormat.currency(locale: 'vi_VN', symbol: 'đ').format(item['giaBanMoi'] ?? 0)}',
                      ),
                    ),
                    DataCell(
                      pct == null
  ...
  Widget _buildCardView(List<dynamic> list, int tabIndex) {
    return ListView.builder(
      ...
      itemBuilder: (context, index) {
        ...
        final pct = tabIndex == 0
            ? item['phanTramGiaBan']
            : item['phanTramThayDoi'];
        final isTang = pct != null && pct > 0;
        final isGiam = pct != null && pct < 0;
        final ma = item['maSP'] ?? item['maSanPham'];
        ...
        return Card(
          ...
          child: ListTile(
            ...
            leading: CircleAvatar(
              backgroundColor: isTang
                  ? Colors.green.shade100
                  : (isGiam ? Colors.red.shade100 : Colors.orange.shade100),
              child: Icon(
                isTang
                    ? Icons.trending_up
                    : (isGiam ? Icons.trending_down : Icons.trending_flat),
                color: isTang
                    ? Colors.green
                    : (isGiam ? Colors.red : Colors.orange),
              ),
            ...
            subtitle: Column(
              ...
              children: [
                ...
                if (tabIndex == 0) ...[
                  Text(
                    'Giá bán: ${item['giaBanTruoc'] != null ? NumberFormat.currency(locale: 'vi_VN', symbol: 'đ').format(item['giaBanTruoc']) : "N/A"} -> ${NumberFormat.currency(locale: 'vi_VN', symbol: 'đ').format(item['giaBanHienTai'] ?? 0)}',
                  ),
                  Text('Tồn kho: ${item['soLuongTon'] ?? 0}'),
                ] else if (tabIndex == 1) ...[
                  Text(
                    'Giá bán: ${item['giaBanCu'] != null ? NumberFormat.currency(locale: 'vi_VN', symbol: 'đ').format(item['giaBanCu']) : "N/A"} -> ${NumberFormat.currency(locale: 'vi_VN', symbol: 'đ').format(item['giaBanMoi'] ?? 0)}',
                  ),
                  Text(
                    'Ngày: ${item['ngayThayDoi'] != null ? DateFormat('dd/MM/yyyy HH:mm').format(DateTime.parse(item['ngayThayDoi'])) : "N/A"} | NV: ${item['tenNhanVien'] ?? "Hệ thống"}',
                  ),
                ] else ...[
                  Text('Số lần thay đổi: ${item['soLanThayDoi'] ?? 0}'),
                  Text(
                    'Giá: ${NumberFormat.currency(locale: 'vi_VN', symbol: 'đ').format(item['giaDauKy'] ?? 0)} -> ${NumberFormat.currency(locale: 'vi_VN', symbol: 'đ').format(item['giaCuoiKy'] ?? 0)}',
                  ),
                ],
              ],
            ),
            trailing: tabIndex == 0
                ? IconButton(
                    icon: Icon(Icons.show_chart, color: AppColors.primaryStart),
                    onPressed: () => _showChartDialog(item['maSanPham'], ten),
                  )
                : const Icon(Icons.arrow_forward_ios, size: 16),
            onTap: tabIndex == 0
                ? () => _showChartDialog(item['maSanPham'], ten)
                : null,
          ),
        );
      },
