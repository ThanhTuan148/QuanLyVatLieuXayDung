// <auto-generated />
using System;
using BuildingMaterialAPI.Data;
using Microsoft.EntityFrameworkCore;
using Microsoft.EntityFrameworkCore.Infrastructure;
using Microsoft.EntityFrameworkCore.Metadata;
using Microsoft.EntityFrameworkCore.Migrations;
using Microsoft.EntityFrameworkCore.Storage.ValueConversion;

#nullable disable

namespace BuildingMaterialAPI.Migrations
{
    [DbContext(typeof(ApplicationDbContext))]
    [Migration("20260514102505_AddFlashSaleQuantityToKhuyenMaiDoiTuong")]
    partial class AddFlashSaleQuantityToKhuyenMaiDoiTuong
    {
        /// <inheritdoc />
        protected override void BuildTargetModel(ModelBuilder modelBuilder)
        {
#pragma warning disable 612, 618
            modelBuilder
                .HasAnnotation("ProductVersion", "8.0.0")
                .HasAnnotation("Relational:MaxIdentifierLength", 128);

            SqlServerModelBuilderExtensions.UseIdentityColumns(modelBuilder);

            modelBuilder.Entity("BuildingMaterialAPI.Models.CTHD", b =>
                {
                    b.Property<int>("MaCTHD")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasColumnName("MaCTHD");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("MaCTHD"));

                    b.Property<string>("DiaChiGiaoHang")
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("DiaChiGiaoHang");

                    b.Property<decimal>("DonGia")
                        .HasColumnType("decimal(18,2)")
                        .HasColumnName("DonGia");

                    b.Property<decimal>("GiamGia")
                        .HasColumnType("decimal(18,2)")
                        .HasColumnName("GiamGia");

                    b.Property<int>("MaHoaDon")
                        .HasColumnType("int")
                        .HasColumnName("MaHoaDon");

                    b.Property<int>("MaSanPham")
                        .HasColumnType("int")
                        .HasColumnName("MaSanPham");

                    b.Property<DateTime>("NgayTao")
                        .HasColumnType("datetime2")
                        .HasColumnName("NgayTao");

                    b.Property<string>("SdtNguoiNhan")
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("SdtNguoiNhan");

                    b.Property<int>("SoLuong")
                        .HasColumnType("int")
                        .HasColumnName("SoLuong");

                    b.Property<string>("TenNguoiNhan")
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("TenNguoiNhan");

                    b.Property<decimal?>("ThanhTien")
                        .HasColumnType("decimal(18,2)")
                        .HasColumnName("ThanhTien");

                    b.HasKey("MaCTHD");

                    b.HasIndex("MaHoaDon");

                    b.HasIndex("MaSanPham");

                    b.ToTable("CTHD");
                });

            modelBuilder.Entity("BuildingMaterialAPI.Models.CTKhoHang", b =>
                {
                    b.Property<int>("MaCTKho")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasColumnName("MaCTKho");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("MaCTKho"));

                    b.Property<int>("MaKhoHang")
                        .HasColumnType("int")
                        .HasColumnName("MaKhoHang");

                    b.Property<int>("MaSanPham")
                        .HasColumnType("int")
                        .HasColumnName("MaSanPham");

                    b.Property<DateTime>("NgayCapNhat")
                        .HasColumnType("datetime2")
                        .HasColumnName("NgayCapNhat");

                    b.Property<DateTime?>("NgayNhapCuoi")
                        .HasColumnType("datetime2")
                        .HasColumnName("NgayNhapCuoi");

                    b.Property<int>("SoLuong")
                        .HasColumnType("int")
                        .HasColumnName("SoLuong");

                    b.Property<int>("SoLuongNhap")
                        .HasColumnType("int")
                        .HasColumnName("SoLuongNhap");

                    b.Property<int>("SoLuongTon")
                        .HasColumnType("int")
                        .HasColumnName("SoLuongTon");

                    b.HasKey("MaCTKho");

                    b.HasIndex("MaKhoHang");

                    b.HasIndex("MaSanPham");

                    b.ToTable("CTKHOHANG");
                });

            modelBuilder.Entity("BuildingMaterialAPI.Models.CTPN", b =>
                {
                    b.Property<int>("MaCTPN")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasColumnName("MaCTPN");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("MaCTPN"));

                    b.Property<decimal>("DonGia")
                        .HasColumnType("decimal(18,2)")
                        .HasColumnName("DonGia");

                    b.Property<string>("GhiChu")
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("GhiChu");

                    b.Property<int?>("MaKhoHang")
                        .HasColumnType("int")
                        .HasColumnName("MaKhoHang");

                    b.Property<int?>("MaNhaCungCap")
                        .HasColumnType("int")
                        .HasColumnName("MaNhaCungCap");

                    b.Property<int>("MaPhieuNhap")
                        .HasColumnType("int")
                        .HasColumnName("MaPhieuNhap");

                    b.Property<int>("MaSanPham")
                        .HasColumnType("int")
                        .HasColumnName("MaSanPham");

                    b.Property<DateTime>("NgayTao")
                        .HasColumnType("datetime2")
                        .HasColumnName("NgayTao");

                    b.Property<int>("SoLuong")
                        .HasColumnType("int")
                        .HasColumnName("SoLuong");

                    b.Property<int>("SoLuongDaNhan")
                        .HasColumnType("int")
                        .HasColumnName("SoLuongDaNhan");

                    b.Property<decimal?>("ThanhTien")
                        .HasColumnType("decimal(18,2)")
                        .HasColumnName("ThanhTien");

                    b.Property<string>("TrangThai")
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("TrangThai");

                    b.HasKey("MaCTPN");

                    b.HasIndex("MaKhoHang");

                    b.HasIndex("MaNhaCungCap");

                    b.HasIndex("MaPhieuNhap");

                    b.HasIndex("MaSanPham");

                    b.ToTable("CTPN");
                });

            modelBuilder.Entity("BuildingMaterialAPI.Models.CTPhieuDoiTra", b =>
                {
                    b.Property<int>("MaCTDT")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasColumnName("MaCTDT");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("MaCTDT"));

                    b.Property<decimal>("DonGia")
                        .HasColumnType("decimal(18,2)")
                        .HasColumnName("DonGia");

                    b.Property<string>("Loai")
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("Loai");

                    b.Property<int>("MaPhieuDT")
                        .HasColumnType("int")
                        .HasColumnName("MaPhieuDT");

                    b.Property<int>("MaSanPham")
                        .HasColumnType("int")
                        .HasColumnName("MaSanPham");

                    b.Property<DateTime>("NgayTao")
                        .HasColumnType("datetime2")
                        .HasColumnName("NgayTao");

                    b.Property<int>("SoLuong")
                        .HasColumnType("int")
                        .HasColumnName("SoLuong");

                    b.Property<decimal?>("ThanhTien")
                        .HasColumnType("decimal(18,2)")
                        .HasColumnName("ThanhTien");

                    b.Property<string>("TrangThai")
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("TrangThai");

                    b.HasKey("MaCTDT");

                    b.HasIndex("MaPhieuDT");

                    b.HasIndex("MaSanPham");

                    b.ToTable("CTPHIEUDOITRA");
                });

            modelBuilder.Entity("BuildingMaterialAPI.Models.CTPhieuGiaoHang", b =>
                {
                    b.Property<int>("MaCTGH")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasColumnName("MaCTGH");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("MaCTGH"));

                    b.Property<string>("GhiChu")
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("GhiChu");

                    b.Property<int?>("MaCTHD")
                        .HasColumnType("int")
                        .HasColumnName("MaCTHD");

                    b.Property<int>("MaPhieuGH")
                        .HasColumnType("int")
                        .HasColumnName("MaPhieuGH");

                    b.Property<int>("MaSanPham")
                        .HasColumnType("int")
                        .HasColumnName("MaSanPham");

                    b.Property<DateTime>("NgayTao")
                        .HasColumnType("datetime2")
                        .HasColumnName("NgayTao");

                    b.Property<int>("SoLuongGiao")
                        .HasColumnType("int")
                        .HasColumnName("SoLuongGiao");

                    b.Property<string>("TrangThai")
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("TrangThai");

                    b.HasKey("MaCTGH");

                    b.HasIndex("MaPhieuGH");

                    b.HasIndex("MaSanPham");

                    b.ToTable("CTPHIEUGIAOHANG");
                });

            modelBuilder.Entity("BuildingMaterialAPI.Models.CTPhieuTraHangNCC", b =>
                {
                    b.Property<int>("MaCTPT")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasColumnName("MaCTPT");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("MaCTPT"));

                    b.Property<decimal>("DonGia")
                        .HasColumnType("decimal(18,2)")
                        .HasColumnName("DonGia");

                    b.Property<int>("MaPhieuTra")
                        .HasColumnType("int")
                        .HasColumnName("MaPhieuTra");

                    b.Property<int>("MaSanPham")
                        .HasColumnType("int")
                        .HasColumnName("MaSanPham");

                    b.Property<int>("SoLuongTra")
                        .HasColumnType("int")
                        .HasColumnName("SoLuongTra");

                    b.Property<decimal?>("ThanhTien")
                        .HasColumnType("decimal(18,2)")
                        .HasColumnName("ThanhTien");

                    b.HasKey("MaCTPT");

                    b.HasIndex("MaPhieuTra");

                    b.HasIndex("MaSanPham");

                    b.ToTable("CT_PHIEUTRAHANG_NCC");
                });

            modelBuilder.Entity("BuildingMaterialAPI.Models.CTPhieuXuatKho", b =>
                {
                    b.Property<int>("MaCTXK")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasColumnName("MaCTXK");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("MaCTXK"));

                    b.Property<decimal?>("DonGiaVon")
                        .HasColumnType("decimal(18,2)")
                        .HasColumnName("DonGiaVon");

                    b.Property<string>("GhiChu")
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("GhiChu");

                    b.Property<int?>("MaKho")
                        .HasColumnType("int")
                        .HasColumnName("MaKho");

                    b.Property<int>("MaPhieuXK")
                        .HasColumnType("int")
                        .HasColumnName("MaPhieuXK");

                    b.Property<int>("MaSanPham")
                        .HasColumnType("int")
                        .HasColumnName("MaSanPham");

                    b.Property<int>("SoLuong")
                        .HasColumnType("int")
                        .HasColumnName("SoLuong");

                    b.Property<int?>("SoLuongThucNhan")
                        .HasColumnType("int")
                        .HasColumnName("SoLuongThucNhan");

                    b.HasKey("MaCTXK");

                    b.HasIndex("MaKho");

                    b.HasIndex("MaPhieuXK");

                    b.HasIndex("MaSanPham");

                    b.ToTable("CTPHIEUXUATKHO");
                });

            modelBuilder.Entity("BuildingMaterialAPI.Models.ChiTietTraNo", b =>
                {
                    b.Property<int>("MaChiTietTN")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasColumnName("MaChiTietTN");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("MaChiTietTN"));

                    b.Property<string>("AnhBangChung")
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("AnhBangChung");

                    b.Property<string>("GhiChu")
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("GhiChu");

                    b.Property<int>("MaCongNo")
                        .HasColumnType("int")
                        .HasColumnName("MaCongNo");

                    b.Property<int?>("MaHoaDon")
                        .HasColumnType("int")
                        .HasColumnName("MaHoaDon");

                    b.Property<int?>("MaNhanVien")
                        .HasColumnType("int")
                        .HasColumnName("MaNhanVien");

                    b.Property<int?>("MaPhieuNhap")
                        .HasColumnType("int")
                        .HasColumnName("MaPhieuNhap");

                    b.Property<string>("MaTT")
                        .IsRequired()
                        .ValueGeneratedOnAddOrUpdate()
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("MaTT");

                    b.Property<DateTime>("NgayTT")
                        .HasColumnType("datetime2")
                        .HasColumnName("NgayTT");

                    b.Property<DateTime>("NgayTao")
                        .HasColumnType("datetime2")
                        .HasColumnName("NgayTao");

                    b.Property<string>("PTTT")
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("PTTT");

                    b.Property<string>("SoGiaoDich")
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("SoGiaoDich");

                    b.Property<decimal>("SoTien")
                        .HasColumnType("decimal(18,2)")
                        .HasColumnName("SoTien");

                    b.Property<string>("TrangThai")
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("TrangThai");

                    b.HasKey("MaChiTietTN");

                    b.HasIndex("MaCongNo");

                    b.HasIndex("MaHoaDon");

                    b.HasIndex("MaNhanVien");

                    b.HasIndex("MaPhieuNhap");

                    b.ToTable("CHITETTRANO");
                });

            modelBuilder.Entity("BuildingMaterialAPI.Models.CongNo", b =>
                {
                    b.Property<int>("MaCongNo")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasColumnName("MaCongNo");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("MaCongNo"));

                    b.Property<string>("GhiChu")
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("GhiChu");

                    b.Property<DateTime?>("HanThanhToan")
                        .HasColumnType("datetime2")
                        .HasColumnName("HanThanhToan");

                    b.Property<decimal>("LaiPhat")
                        .HasColumnType("decimal(18,2)")
                        .HasColumnName("LaiPhat");

                    b.Property<string>("LoaiCongNo")
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("LoaiCongNo");

                    b.Property<string>("MaCN")
                        .IsRequired()
                        .ValueGeneratedOnAddOrUpdate()
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("MaCN");

                    b.Property<int?>("MaHoaDon")
                        .HasColumnType("int")
                        .HasColumnName("MaHoaDon");

                    b.Property<int?>("MaKhachHang")
                        .HasColumnType("int")
                        .HasColumnName("MaKhachHang");

                    b.Property<int?>("MaNhaCungCap")
                        .HasColumnType("int")
                        .HasColumnName("MaNhaCungCap");

                    b.Property<int?>("MaPhieuNhap")
                        .HasColumnType("int")
                        .HasColumnName("MaPhieuNhap");

                    b.Property<DateTime>("NgayCapNhat")
                        .HasColumnType("datetime2")
                        .HasColumnName("NgayCapNhat");

                    b.Property<DateTime?>("NgayNhacNoEmail")
                        .HasColumnType("datetime2")
                        .HasColumnName("NgayNhacNoEmail");

                    b.Property<DateTime>("NgayTao")
                        .HasColumnType("datetime2")
                        .HasColumnName("NgayTao");

                    b.Property<decimal?>("SoTienConLai")
                        .HasColumnType("decimal(18,2)")
                        .HasColumnName("SoTienConLai");

                    b.Property<decimal>("SoTienDaTra")
                        .HasColumnType("decimal(18,2)")
                        .HasColumnName("SoTienDaTra");

                    b.Property<decimal>("SoTienNo")
                        .HasColumnType("decimal(18,2)")
                        .HasColumnName("SoTienNo");

                    b.Property<string>("TrangThai")
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("TrangThai");

                    b.HasKey("MaCongNo");

                    b.HasIndex("MaHoaDon");

                    b.HasIndex("MaKhachHang");

                    b.HasIndex("MaNhaCungCap");

                    b.HasIndex("MaPhieuNhap");

                    b.ToTable("CONGNO");
                });

            modelBuilder.Entity("BuildingMaterialAPI.Models.DanhGia", b =>
                {
                    b.Property<int>("MaDanhGia")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasColumnName("MaDanhGia");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("MaDanhGia"));

                    b.Property<string>("HinhAnh")
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("HinhAnh");

                    b.Property<int?>("MaHoaDon")
                        .HasColumnType("int")
                        .HasColumnName("MaHoaDon");

                    b.Property<int>("MaKhachHang")
                        .HasColumnType("int")
                        .HasColumnName("MaKhachHang");

                    b.Property<int>("MaSanPham")
                        .HasColumnType("int")
                        .HasColumnName("MaSanPham");

                    b.Property<DateTime>("NgayTao")
                        .HasColumnType("datetime2")
                        .HasColumnName("NgayTao");

                    b.Property<string>("NoiDung")
                        .HasMaxLength(1000)
                        .HasColumnType("nvarchar(1000)")
                        .HasColumnName("NoiDung");

                    b.Property<int>("SoSao")
                        .HasColumnType("int")
                        .HasColumnName("SoSao");

                    b.Property<bool>("TrangThai")
                        .HasColumnType("bit")
                        .HasColumnName("TrangThai");

                    b.Property<string>("Video")
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("Video");

                    b.HasKey("MaDanhGia");

                    b.HasIndex("MaHoaDon");

                    b.HasIndex("MaKhachHang");

                    b.HasIndex("MaSanPham");

                    b.ToTable("DANHGIA");
                });

            modelBuilder.Entity("BuildingMaterialAPI.Models.HoaDon", b =>
                {
                    b.Property<int>("MaHoaDon")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasColumnName("MaHoaDon");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("MaHoaDon"));

                    b.Property<string>("AnhBangChung")
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("AnhBangChung");

                    b.Property<string>("DiaChiGiaoHang")
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("DiaChiGiaoHang");

                    b.Property<string>("EmailNguoiNhan")
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("EmailNguoiNhan");

                    b.Property<string>("GhiChu")
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("GhiChu");

                    b.Property<decimal>("GiamGia")
                        .HasColumnType("decimal(18,2)")
                        .HasColumnName("GiamGia");

                    b.Property<string>("MaHD")
                        .IsRequired()
                        .ValueGeneratedOnAddOrUpdate()
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("MaHD");

                    b.Property<int?>("MaKhachHang")
                        .HasColumnType("int")
                        .HasColumnName("MaKhachHang");

                    b.Property<int?>("MaKhuyenMai")
                        .HasColumnType("int")
                        .HasColumnName("MaKhuyenMai");

                    b.Property<int?>("MaNhanVien")
                        .HasColumnType("int")
                        .HasColumnName("MaNhanVien");

                    b.Property<DateTime>("NgayCapNhat")
                        .HasColumnType("datetime2")
                        .HasColumnName("NgayCapNhat");

                    b.Property<DateTime?>("NgayGiao")
                        .HasColumnType("datetime2")
                        .HasColumnName("NgayGiao");

                    b.Property<DateTime>("NgayLap")
                        .HasColumnType("datetime2")
                        .HasColumnName("NgayLap");

                    b.Property<DateTime>("NgayTao")
                        .HasColumnType("datetime2")
                        .HasColumnName("NgayTao");

                    b.Property<string>("PTTT")
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("PTTT");

                    b.Property<decimal>("PhiVanChuyen")
                        .HasColumnType("decimal(18,2)")
                        .HasColumnName("PhiVanChuyen");

                    b.Property<string>("SdtNguoiNhan")
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("SdtNguoiNhan");

                    b.Property<decimal>("SoTienPhaiThu")
                        .HasColumnType("decimal(18,2)")
                        .HasColumnName("SoTienPhaiThu");

                    b.Property<string>("TenNguoiNhan")
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("TenNguoiNhan");

                    b.Property<decimal?>("ThanhToan")
                        .HasColumnType("decimal(18,2)")
                        .HasColumnName("ThanhToan");

                    b.Property<decimal?>("TongTien")
                        .HasColumnType("decimal(18,2)")
                        .HasColumnName("TongTien");

                    b.Property<string>("TrangThai")
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("TrangThai");

                    b.Property<string>("VatAddress")
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("VatAddress");

                    b.Property<string>("VatBudgetCode")
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("VatBudgetCode");

                    b.Property<string>("VatBuyerName")
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("VatBuyerName");

                    b.Property<string>("VatCompanyAddress")
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("VatCompanyAddress");

                    b.Property<string>("VatCompanyName")
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("VatCompanyName");

                    b.Property<string>("VatEmail")
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("VatEmail");

                    b.Property<string>("VatIdCard")
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("VatIdCard");

                    b.Property<string>("VatPassport")
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("VatPassport");

                    b.Property<string>("VatTaxId")
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("VatTaxId");

                    b.Property<string>("VatType")
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("VatType");

                    b.Property<bool>("YeuCauVat")
                        .HasColumnType("bit")
                        .HasColumnName("YeuCauVat");

                    b.HasKey("MaHoaDon");

                    b.HasIndex("MaKhachHang");

                    b.HasIndex("MaKhuyenMai");

                    b.HasIndex("MaNhanVien");

                    b.ToTable("HOADON");
                });

            modelBuilder.Entity("BuildingMaterialAPI.Models.KhachHang", b =>
                {
                    b.Property<int>("MaKhachHang")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasColumnName("MaKhachHang");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("MaKhachHang"));

                    b.Property<string>("AnhDaiDien")
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("AnhDaiDien");

                    b.Property<string>("CCCD")
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("CCCD");

                    b.Property<string>("DiaChi")
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("DiaChi");

                    b.Property<string>("Email")
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("Email");

                    b.Property<string>("GioiTinh")
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("GioiTinh");

                    b.Property<string>("HangThanhVien")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("HangThanhVien");

                    b.Property<string>("LoaiKH")
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("LoaiKH");

                    b.Property<string>("MaKH")
                        .IsRequired()
                        .ValueGeneratedOnAddOrUpdate()
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("MaKH");

                    b.Property<string>("MaSoThue")
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("MaSoThue");

                    b.Property<int?>("MaTaiKhoan")
                        .HasColumnType("int")
                        .HasColumnName("MaTaiKhoan");

                    b.Property<DateTime>("NgayCapNhat")
                        .HasColumnType("datetime2")
                        .HasColumnName("NgayCapNhat");

                    b.Property<DateTime?>("NgaySinh")
                        .HasColumnType("datetime2")
                        .HasColumnName("NgaySinh");

                    b.Property<DateTime>("NgayTao")
                        .HasColumnType("datetime2")
                        .HasColumnName("NgayTao");

                    b.Property<string>("NguoiLienHe")
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("NguoiLienHe");

                    b.Property<string>("Sdt")
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("Sdt");

                    b.Property<string>("TenKH")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("TenKH");

                    b.Property<decimal>("TongChiTieu")
                        .HasColumnType("decimal(18,2)")
                        .HasColumnName("TongChiTieu");

                    b.Property<bool>("TrangThai")
                        .HasColumnType("bit")
                        .HasColumnName("TrangThai");

                    b.HasKey("MaKhachHang");

                    b.HasIndex("MaTaiKhoan")
                        .IsUnique()
                        .HasFilter("[MaTaiKhoan] IS NOT NULL");

                    b.ToTable("KHACHHANG");
                });

            modelBuilder.Entity("BuildingMaterialAPI.Models.KhoHang", b =>
                {
                    b.Property<int>("MaKhoHang")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasColumnName("MaKhoHang");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("MaKhoHang"));

                    b.Property<string>("DiaChi")
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("DiaChi");

                    b.Property<string>("GhiChu")
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("GhiChu");

                    b.Property<string>("LoaiKho")
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("LoaiKho");

                    b.Property<string>("MaKho")
                        .ValueGeneratedOnAddOrUpdate()
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("MaKho");

                    b.Property<DateTime?>("NgayCapNhat")
                        .HasColumnType("datetime2")
                        .HasColumnName("NgayCapNhat");

                    b.Property<DateTime?>("NgayTao")
                        .HasColumnType("datetime2")
                        .HasColumnName("NgayTao");

                    b.Property<string>("TenKho")
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("TenKho");

                    b.Property<bool>("TrangThai")
                        .HasColumnType("bit")
                        .HasColumnName("TrangThai");

                    b.HasKey("MaKhoHang");

                    b.ToTable("KHOHANG");
                });

            modelBuilder.Entity("BuildingMaterialAPI.Models.KhuyenMai", b =>
                {
                    b.Property<int>("MaKhuyenMai")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasColumnName("MaKhuyenMai");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("MaKhuyenMai"));

                    b.Property<decimal>("DonHangToiThieu")
                        .HasColumnType("decimal(18,2)")
                        .HasColumnName("DonHangToiThieu");

                    b.Property<decimal>("GiaTriGiam")
                        .HasColumnType("decimal(18,2)")
                        .HasColumnName("GiaTriGiam");

                    b.Property<decimal?>("GiamToiDa")
                        .HasColumnType("decimal(18,2)")
                        .HasColumnName("GiamToiDa");

                    b.Property<string>("HangThanhVien")
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("HangThanhVien");

                    b.Property<string>("HinhAnh")
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("HinhAnh");

                    b.Property<string>("LoaiGiamGia")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("LoaiGiamGia");

                    b.Property<string>("LoaiKM")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("LoaiKM");

                    b.Property<string>("MaApDung")
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("MaApDung");

                    b.Property<string>("MaKM")
                        .ValueGeneratedOnAddOrUpdate()
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("MaKM");

                    b.Property<string>("MoTa")
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("MoTa");

                    b.Property<DateTime>("NgayCapNhat")
                        .HasColumnType("datetime2")
                        .HasColumnName("NgayCapNhat");

                    b.Property<DateTime>("NgayTao")
                        .HasColumnType("datetime2")
                        .HasColumnName("NgayTao");

                    b.Property<int>("SoLuongDaDung")
                        .HasColumnType("int")
                        .HasColumnName("SoLuongDaDung");

                    b.Property<int?>("SoLuongToiDa")
                        .HasColumnType("int")
                        .HasColumnName("SoLuongToiDa");

                    b.Property<string>("TenKM")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("TenKM");

                    b.Property<DateTime>("ThoiGianBatDau")
                        .HasColumnType("datetime2")
                        .HasColumnName("ThoiGianBatDau");

                    b.Property<DateTime>("ThoiGianKetThuc")
                        .HasColumnType("datetime2")
                        .HasColumnName("ThoiGianKetThuc");

                    b.Property<bool>("TrangThai")
                        .HasColumnType("bit")
                        .HasColumnName("TrangThai");

                    b.HasKey("MaKhuyenMai");

                    b.ToTable("KHUYENMAI");
                });

            modelBuilder.Entity("BuildingMaterialAPI.Models.KhuyenMaiDoiTuong", b =>
                {
                    b.Property<int>("MaKMDT")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasColumnName("MaKMDT");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("MaKMDT"));

                    b.Property<decimal?>("GiaKhuyenMai")
                        .HasColumnType("decimal(18,2)")
                        .HasColumnName("GiaKhuyenMai");

                    b.Property<int>("MaKhuyenMai")
                        .HasColumnType("int")
                        .HasColumnName("MaKhuyenMai");

                    b.Property<int?>("MaLoaiSP")
                        .HasColumnType("int")
                        .HasColumnName("MaLoaiSP");

                    b.Property<int?>("MaSanPham")
                        .HasColumnType("int")
                        .HasColumnName("MaSanPham");

                    b.Property<int>("SoLuongDaBan")
                        .HasColumnType("int")
                        .HasColumnName("SoLuongDaBan");

                    b.Property<int?>("SoLuongKhuyenMai")
                        .HasColumnType("int")
                        .HasColumnName("SoLuongKhuyenMai");

                    b.HasKey("MaKMDT");

                    b.HasIndex("MaKhuyenMai");

                    b.HasIndex("MaLoaiSP");

                    b.HasIndex("MaSanPham");

                    b.ToTable("KHUYENMAI_DOITUONG");
                });

            modelBuilder.Entity("BuildingMaterialAPI.Models.LichHenTraNo", b =>
                {
                    b.Property<int>("MaHen")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasColumnName("MaHen");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("MaHen"));

                    b.Property<string>("GhiChu")
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("GhiChu");

                    b.Property<int>("MaCongNo")
                        .HasColumnType("int")
                        .HasColumnName("MaCongNo");

                    b.Property<DateTime>("NgayHen")
                        .HasColumnType("datetime2")
                        .HasColumnName("NgayHen");

                    b.Property<DateTime>("NgayTao")
                        .HasColumnType("datetime2")
                        .HasColumnName("NgayTao");

                    b.Property<decimal>("SoTienDuKien")
                        .HasColumnType("decimal(18,2)")
                        .HasColumnName("SoTienDuKien");

                    b.Property<string>("TrangThai")
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("TrangThai");

                    b.HasKey("MaHen");

                    b.HasIndex("MaCongNo");

                    b.ToTable("LICHHENTRANO");
                });

            modelBuilder.Entity("BuildingMaterialAPI.Models.LichSuGia", b =>
                {
                    b.Property<int>("MaLSG")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasColumnName("MaLSG");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("MaLSG"));

                    b.Property<decimal?>("GiaBanCu")
                        .HasColumnType("decimal(18,2)")
                        .HasColumnName("GiaBanCu");

                    b.Property<decimal>("GiaBanMoi")
                        .HasColumnType("decimal(18,2)")
                        .HasColumnName("GiaBanMoi");

                    b.Property<decimal?>("GiaNhapCu")
                        .HasColumnType("decimal(18,2)")
                        .HasColumnName("GiaNhapCu");

                    b.Property<decimal?>("GiaNhapMoi")
                        .HasColumnType("decimal(18,2)")
                        .HasColumnName("GiaNhapMoi");

                    b.Property<string>("LyDo")
                        .HasMaxLength(500)
                        .HasColumnType("nvarchar(500)")
                        .HasColumnName("LyDo");

                    b.Property<int?>("MaNhanVien")
                        .HasColumnType("int")
                        .HasColumnName("MaNhanVien");

                    b.Property<int>("MaSanPham")
                        .HasColumnType("int")
                        .HasColumnName("MaSanPham");

                    b.Property<DateTime>("NgayThayDoi")
                        .HasColumnType("datetime2")
                        .HasColumnName("NgayThayDoi");

                    b.Property<string>("NguonThayDoi")
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)")
                        .HasColumnName("NguonThayDoi");

                    b.HasKey("MaLSG");

                    b.HasIndex("MaNhanVien");

                    b.HasIndex("MaSanPham");

                    b.ToTable("LICHSUGIA");
                });

            modelBuilder.Entity("BuildingMaterialAPI.Models.LichSuGiaoHang", b =>
                {
                    b.Property<int>("MaLichSu")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasColumnName("MaLichSu");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("MaLichSu"));

                    b.Property<string>("HinhAnhXacNhan")
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("HinhAnhXacNhan");

                    b.Property<int?>("MaNguoiThucHien")
                        .HasColumnType("int")
                        .HasColumnName("MaNguoiThucHien");

                    b.Property<int>("MaPhieuGH")
                        .HasColumnType("int")
                        .HasColumnName("MaPhieuGH");

                    b.Property<DateTime>("NgayTao")
                        .HasColumnType("datetime2")
                        .HasColumnName("NgayTao");

                    b.Property<string>("NoiDungThayDoi")
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("NoiDungThayDoi");

                    b.Property<string>("TrangThaiCu")
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("TrangThaiCu");

                    b.Property<string>("TrangThaiMoi")
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("TrangThaiMoi");

                    b.Property<string>("ViTriCapNhat")
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("ViTriCapNhat");

                    b.HasKey("MaLichSu");

                    b.HasIndex("MaPhieuGH");

                    b.ToTable("LICHSUGIAOHANG");
                });

            modelBuilder.Entity("BuildingMaterialAPI.Models.LichSuHoaDon", b =>
                {
                    b.Property<int>("MaLichSu")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasColumnName("MaLichSu");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("MaLichSu"));

                    b.Property<int>("MaHoaDon")
                        .HasColumnType("int")
                        .HasColumnName("MaHoaDon");

                    b.Property<int?>("MaNguoiThucHien")
                        .HasColumnType("int")
                        .HasColumnName("MaNguoiThucHien");

                    b.Property<DateTime>("NgayTao")
                        .HasColumnType("datetime2")
                        .HasColumnName("NgayTao");

                    b.Property<string>("NoiDungThayDoi")
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("NoiDungThayDoi");

                    b.Property<string>("TrangThaiCu")
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("TrangThaiCu");

                    b.Property<string>("TrangThaiMoi")
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("TrangThaiMoi");

                    b.HasKey("MaLichSu");

                    b.HasIndex("MaHoaDon");

                    b.HasIndex("MaNguoiThucHien");

                    b.ToTable("LICHSUHOADON");
                });

            modelBuilder.Entity("BuildingMaterialAPI.Models.LichSuPhieuNhap", b =>
                {
                    b.Property<int>("MaLichSu")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("MaLichSu"));

                    b.Property<int?>("MaNguoiThucHien")
                        .HasColumnType("int");

                    b.Property<int>("MaPhieuNhap")
                        .HasColumnType("int");

                    b.Property<DateTime>("NgayThayDoi")
                        .HasColumnType("datetime2");

                    b.Property<string>("NoiDungThayDoi")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("TrangThaiCu")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("TrangThaiMoi")
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("MaLichSu");

                    b.HasIndex("MaNguoiThucHien");

                    b.HasIndex("MaPhieuNhap");

                    b.ToTable("LICHSUPHIEUNHAP");
                });

            modelBuilder.Entity("BuildingMaterialAPI.Models.LichSuPhieuXuatKho", b =>
                {
                    b.Property<int>("MaLichSu")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasColumnName("MaLichSu");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("MaLichSu"));

                    b.Property<int?>("MaNguoiThucHien")
                        .HasColumnType("int")
                        .HasColumnName("MaNguoiThucHien");

                    b.Property<int>("MaPhieuXK")
                        .HasColumnType("int")
                        .HasColumnName("MaPhieuXK");

                    b.Property<DateTime>("NgayTao")
                        .HasColumnType("datetime2")
                        .HasColumnName("NgayTao");

                    b.Property<string>("NoiDungThayDoi")
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("NoiDungThayDoi");

                    b.Property<string>("TrangThaiCu")
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("TrangThaiCu");

                    b.Property<string>("TrangThaiMoi")
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("TrangThaiMoi");

                    b.HasKey("MaLichSu");

                    b.HasIndex("MaNguoiThucHien");

                    b.HasIndex("MaPhieuXK");

                    b.ToTable("LICHSUPHIEUXUATKHO");
                });

            modelBuilder.Entity("BuildingMaterialAPI.Models.LichSuThangHang", b =>
                {
                    b.Property<int>("MaLichSu")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasColumnName("MaLichSu");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("MaLichSu"));

                    b.Property<string>("HangCu")
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("HangCu");

                    b.Property<string>("HangMoi")
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("HangMoi");

                    b.Property<string>("LyDo")
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("LyDo");

                    b.Property<int>("MaKhachHang")
                        .HasColumnType("int")
                        .HasColumnName("MaKhachHang");

                    b.Property<DateTime>("NgayThayDoi")
                        .HasColumnType("datetime2")
                        .HasColumnName("NgayThayDoi");

                    b.Property<decimal>("TongChiTieuHienTai")
                        .HasColumnType("decimal(18,2)")
                        .HasColumnName("TongChiTieuHienTai");

                    b.HasKey("MaLichSu");

                    b.HasIndex("MaKhachHang");

                    b.ToTable("LICHSUTHANGHANG");
                });

            modelBuilder.Entity("BuildingMaterialAPI.Models.LoaiSanPham", b =>
                {
                    b.Property<int>("MaLoaiSP")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasColumnName("MaLoaiSP");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("MaLoaiSP"));

                    b.Property<string>("HinhAnh")
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("HinhAnh");

                    b.Property<string>("MaLoai")
                        .IsRequired()
                        .ValueGeneratedOnAddOrUpdate()
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("MaLoai");

                    b.Property<string>("MoTa")
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("MoTa");

                    b.Property<DateTime>("NgayCapNhat")
                        .HasColumnType("datetime2")
                        .HasColumnName("NgayCapNhat");

                    b.Property<DateTime>("NgayTao")
                        .HasColumnType("datetime2")
                        .HasColumnName("NgayTao");

                    b.Property<string>("TenLoai")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("TenLoai");

                    b.HasKey("MaLoaiSP");

                    b.ToTable("LOAISANPHAM");
                });

            modelBuilder.Entity("BuildingMaterialAPI.Models.NhaCungCap", b =>
                {
                    b.Property<int>("MaNhaCungCap")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasColumnName("MaNhaCungCap");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("MaNhaCungCap"));

                    b.Property<string>("DiaChi")
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("DiaChi");

                    b.Property<string>("Email")
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("Email");

                    b.Property<string>("MaNCC")
                        .IsRequired()
                        .ValueGeneratedOnAddOrUpdate()
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("MaNCC");

                    b.Property<string>("MaSoThue")
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("MaSoThue");

                    b.Property<DateTime>("NgayCapNhat")
                        .HasColumnType("datetime2")
                        .HasColumnName("NgayCapNhat");

                    b.Property<DateTime>("NgayTao")
                        .HasColumnType("datetime2")
                        .HasColumnName("NgayTao");

                    b.Property<string>("NguoiLienHe")
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("NguoiLienHe");

                    b.Property<string>("Sdt")
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("Sdt");

                    b.Property<string>("TenNCC")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("TenNCC");

                    b.Property<string>("ThanhPho")
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("ThanhPho");

                    b.Property<bool>("TrangThai")
                        .HasColumnType("bit")
                        .HasColumnName("TrangThai");

                    b.HasKey("MaNhaCungCap");

                    b.ToTable("NHACUNGCAP");
                });

            modelBuilder.Entity("BuildingMaterialAPI.Models.NhaCungCapSanPham", b =>
                {
                    b.Property<int>("MaNCCSP")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasColumnName("MaNCCSP");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("MaNCCSP"));

                    b.Property<decimal>("GiaCungCap")
                        .HasColumnType("decimal(18,2)")
                        .HasColumnName("GiaCungCap");

                    b.Property<int>("MaNCC")
                        .HasColumnType("int")
                        .HasColumnName("MaNCC");

                    b.Property<int>("MaSanPham")
                        .HasColumnType("int")
                        .HasColumnName("MaSanPham");

                    b.Property<DateTime>("NgayCapNhat")
                        .HasColumnType("datetime2")
                        .HasColumnName("NgayCapNhat");

                    b.HasKey("MaNCCSP");

                    b.HasIndex("MaNCC");

                    b.HasIndex("MaSanPham");

                    b.ToTable("NHACUNGCAP_SANPHAM");
                });

            modelBuilder.Entity("BuildingMaterialAPI.Models.NhanVien", b =>
                {
                    b.Property<int>("MaNhanVien")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasColumnName("MaNhanVien");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("MaNhanVien"));

                    b.Property<string>("ChuKy")
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("ChuKy");

                    b.Property<string>("DiaChi")
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("DiaChi");

                    b.Property<string>("Email")
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("Email");

                    b.Property<string>("MaNV")
                        .IsRequired()
                        .ValueGeneratedOnAddOrUpdate()
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("MaNV");

                    b.Property<int?>("MaTaiKhoan")
                        .HasColumnType("int")
                        .HasColumnName("MaTaiKhoan");

                    b.Property<DateTime>("NgayCapNhat")
                        .HasColumnType("datetime2")
                        .HasColumnName("NgayCapNhat");

                    b.Property<DateTime>("NgayTao")
                        .HasColumnType("datetime2")
                        .HasColumnName("NgayTao");

                    b.Property<string>("Sdt")
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("Sdt");

                    b.Property<string>("SucChuaToiDa")
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("SucChuaToiDa");

                    b.Property<string>("TenNV")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("TenNV");

                    b.Property<bool>("TrangThai")
                        .HasColumnType("bit")
                        .HasColumnName("TrangThai");

                    b.HasKey("MaNhanVien");

                    b.HasIndex("MaTaiKhoan")
                        .IsUnique()
                        .HasFilter("[MaTaiKhoan] IS NOT NULL");

                    b.ToTable("NHANVIEN");
                });

            modelBuilder.Entity("BuildingMaterialAPI.Models.NhanVienModuleQuyen", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasColumnName("Id");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<bool>("CoTheSua")
                        .HasColumnType("bit")
                        .HasColumnName("CoTheSua");

                    b.Property<bool>("CoTheTao")
                        .HasColumnType("bit")
                        .HasColumnName("CoTheTao");

                    b.Property<bool>("CoTheXem")
                        .HasColumnType("bit")
                        .HasColumnName("CoTheXem");

                    b.Property<bool>("CoTheXoa")
                        .HasColumnType("bit")
                        .HasColumnName("CoTheXoa");

                    b.Property<int>("MaNhanVien")
                        .HasColumnType("int")
                        .HasColumnName("MaNhanVien");

                    b.Property<string>("Module")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("Module");

                    b.Property<DateTime>("NgayCapNhat")
                        .HasColumnType("datetime2")
                        .HasColumnName("NgayCapNhat");

                    b.Property<string>("TenModule")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("TenModule");

                    b.HasKey("Id");

                    b.HasIndex("MaNhanVien");

                    b.ToTable("NHANVIEN_MODULE_QUYEN");
                });

            modelBuilder.Entity("BuildingMaterialAPI.Models.NhatKy", b =>
                {
                    b.Property<int>("MaNhatKy")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasColumnName("MaNhatKy");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("MaNhatKy"));

                    b.Property<string>("GiaTriCu")
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("GiaTriCu");

                    b.Property<string>("GiaTriMoi")
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("GiaTriMoi");

                    b.Property<string>("HanhDong")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("HanhDong");

                    b.Property<int?>("MaBanGhi")
                        .HasColumnType("int")
                        .HasColumnName("MaBanGhi");

                    b.Property<int>("MaTaiKhoan")
                        .HasColumnType("int")
                        .HasColumnName("MaTaiKhoan");

                    b.Property<string>("TenBang")
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("TenBang");

                    b.Property<DateTime>("ThoiGian")
                        .HasColumnType("datetime2")
                        .HasColumnName("ThoiGian");

                    b.HasKey("MaNhatKy");

                    b.HasIndex("MaTaiKhoan");

                    b.ToTable("NHATKY");
                });

            modelBuilder.Entity("BuildingMaterialAPI.Models.PhanQuyen", b =>
                {
                    b.Property<int>("MaPhanQuyen")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasColumnName("MaPhanQuyen");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("MaPhanQuyen"));

                    b.Property<int>("MaQuyen")
                        .HasColumnType("int")
                        .HasColumnName("MaQuyen");

                    b.Property<int>("MaVaiTro")
                        .HasColumnType("int")
                        .HasColumnName("MaVaiTro");

                    b.HasKey("MaPhanQuyen");

                    b.HasIndex("MaQuyen");

                    b.HasIndex("MaVaiTro");

                    b.ToTable("PHANQUYEN");
                });

            modelBuilder.Entity("BuildingMaterialAPI.Models.PhieuDoiTra", b =>
                {
                    b.Property<int>("MaPhieuDT")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasColumnName("MaPhieuDT");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("MaPhieuDT"));

                    b.Property<string>("GhiChu")
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("GhiChu");

                    b.Property<string>("HinhAnhMinhChung")
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("HinhAnhMinhChung");

                    b.Property<string>("Loai")
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("Loai");

                    b.Property<string>("LoiDo")
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("LoiDo");

                    b.Property<string>("LyDo")
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("LyDo");

                    b.Property<string>("MaDT")
                        .IsRequired()
                        .ValueGeneratedOnAddOrUpdate()
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("MaDT");

                    b.Property<int>("MaHoaDon")
                        .HasColumnType("int")
                        .HasColumnName("MaHoaDon");

                    b.Property<int>("MaNhanVien")
                        .HasColumnType("int")
                        .HasColumnName("MaNhanVien");

                    b.Property<DateTime>("NgayCapNhat")
                        .HasColumnType("datetime2")
                        .HasColumnName("NgayCapNhat");

                    b.Property<DateTime>("NgayDT")
                        .HasColumnType("datetime2")
                        .HasColumnName("NgayDT");

                    b.Property<DateTime>("NgayTao")
                        .HasColumnType("datetime2")
                        .HasColumnName("NgayTao");

                    b.Property<decimal?>("PhiVanChuyenMoi")
                        .HasColumnType("decimal(18,2)")
                        .HasColumnName("PhiVanChuyenMoi");

                    b.Property<decimal?>("TongTienHoan")
                        .HasColumnType("decimal(18,2)")
                        .HasColumnName("TongTienHoan");

                    b.Property<string>("TrangThai")
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("TrangThai");

                    b.Property<string>("TrangThaiNhapKho")
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("TrangThaiNhapKho");

                    b.HasKey("MaPhieuDT");

                    b.HasIndex("MaHoaDon");

                    b.HasIndex("MaNhanVien");

                    b.ToTable("PHIEUDOITRA");
                });

            modelBuilder.Entity("BuildingMaterialAPI.Models.PhieuGiaoHang", b =>
                {
                    b.Property<int>("MaPhieuGH")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasColumnName("MaPhieuGH");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("MaPhieuGH"));

                    b.Property<string>("DiaChi")
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("DiaChi");

                    b.Property<string>("GhiChu")
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("GhiChu");

                    b.Property<decimal?>("Lat")
                        .HasColumnType("decimal(18,2)")
                        .HasColumnName("Lat");

                    b.Property<decimal?>("Lng")
                        .HasColumnType("decimal(18,2)")
                        .HasColumnName("Lng");

                    b.Property<string>("MaGH")
                        .IsRequired()
                        .ValueGeneratedOnAddOrUpdate()
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("MaGH");

                    b.Property<int?>("MaHoaDon")
                        .HasColumnType("int")
                        .HasColumnName("MaHoaDon");

                    b.Property<int?>("MaNhanVien")
                        .HasColumnType("int")
                        .HasColumnName("MaNhanVien");

                    b.Property<DateTime>("NgayCapNhat")
                        .HasColumnType("datetime2")
                        .HasColumnName("NgayCapNhat");

                    b.Property<DateTime>("NgayGiao")
                        .HasColumnType("datetime2")
                        .HasColumnName("NgayGiao");

                    b.Property<DateTime?>("NgayGiaoDuKien")
                        .HasColumnType("datetime2")
                        .HasColumnName("NgayGiaoDuKien");

                    b.Property<DateTime?>("NgayGiaoThucTe")
                        .HasColumnType("datetime2")
                        .HasColumnName("NgayGiaoThucTe");

                    b.Property<DateTime>("NgayTao")
                        .HasColumnType("datetime2")
                        .HasColumnName("NgayTao");

                    b.Property<string>("NguoiGiao")
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("NguoiGiao");

                    b.Property<decimal?>("SoTienThu")
                        .HasColumnType("decimal(18,2)")
                        .HasColumnName("SoTienThu");

                    b.Property<string>("TrangThai")
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("TrangThai");

                    b.Property<string>("ViTriHienTai")
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("ViTriHienTai");

                    b.HasKey("MaPhieuGH");

                    b.HasIndex("MaHoaDon");

                    b.HasIndex("MaNhanVien");

                    b.ToTable("PHIEUGIAOHANG");
                });

            modelBuilder.Entity("BuildingMaterialAPI.Models.PhieuNhap", b =>
                {
                    b.Property<int>("MaPhieuNhap")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasColumnName("MaPhieuNhap");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("MaPhieuNhap"));

                    b.Property<string>("GhiChu")
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("GhiChu");

                    b.Property<int>("MaNhaCungCap")
                        .HasColumnType("int")
                        .HasColumnName("MaNhaCungCap");

                    b.Property<int>("MaNhanVien")
                        .HasColumnType("int")
                        .HasColumnName("MaNhanVien");

                    b.Property<string>("MaPN")
                        .IsRequired()
                        .ValueGeneratedOnAddOrUpdate()
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("MaPN");

                    b.Property<DateTime>("NgayCapNhat")
                        .HasColumnType("datetime2")
                        .HasColumnName("NgayCapNhat");

                    b.Property<DateTime?>("NgayGiaoHang")
                        .HasColumnType("datetime2")
                        .HasColumnName("NgayGiaoHang");

                    b.Property<DateTime>("NgayNhap")
                        .HasColumnType("datetime2")
                        .HasColumnName("NgayNhap");

                    b.Property<DateTime>("NgayTao")
                        .HasColumnType("datetime2")
                        .HasColumnName("NgayTao");

                    b.Property<decimal?>("ThanhToan")
                        .HasColumnType("decimal(18,2)")
                        .HasColumnName("ThanhToan");

                    b.Property<decimal?>("TongTien")
                        .HasColumnType("decimal(18,2)")
                        .HasColumnName("TongTien");

                    b.Property<string>("TrangThai")
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("TrangThai");

                    b.HasKey("MaPhieuNhap");

                    b.HasIndex("MaNhaCungCap");

                    b.HasIndex("MaNhanVien");

                    b.ToTable("PHIEUNHAP");
                });

            modelBuilder.Entity("BuildingMaterialAPI.Models.PhieuTraHangNCC", b =>
                {
                    b.Property<int>("MaPhieuTra")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasColumnName("MaPhieuTra");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("MaPhieuTra"));

                    b.Property<string>("GhiChu")
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("GhiChu");

                    b.Property<string>("LyDo")
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("LyDo");

                    b.Property<int>("MaNhanVien")
                        .HasColumnType("int")
                        .HasColumnName("MaNhanVien");

                    b.Property<string>("MaPT")
                        .IsRequired()
                        .ValueGeneratedOnAddOrUpdate()
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("MaPT");

                    b.Property<int>("MaPhieuNhap")
                        .HasColumnType("int")
                        .HasColumnName("MaPhieuNhap");

                    b.Property<DateTime>("NgayCapNhat")
                        .HasColumnType("datetime2")
                        .HasColumnName("NgayCapNhat");

                    b.Property<DateTime>("NgayTao")
                        .HasColumnType("datetime2")
                        .HasColumnName("NgayTao");

                    b.Property<DateTime>("NgayTra")
                        .HasColumnType("datetime2")
                        .HasColumnName("NgayTra");

                    b.Property<decimal?>("TongTienHoan")
                        .HasColumnType("decimal(18,2)")
                        .HasColumnName("TongTienHoan");

                    b.Property<string>("TrangThai")
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("TrangThai");

                    b.HasKey("MaPhieuTra");

                    b.HasIndex("MaNhanVien");

                    b.HasIndex("MaPhieuNhap");

                    b.ToTable("PHIEUTRAHANG_NCC");
                });

            modelBuilder.Entity("BuildingMaterialAPI.Models.PhieuXuatKho", b =>
                {
                    b.Property<int>("MaPhieuXK")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasColumnName("MaPhieuXK");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("MaPhieuXK"));

                    b.Property<string>("ChuKyNguoiLap")
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("ChuKyNguoiLap");

                    b.Property<string>("ChuKyNguoiNhan")
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("ChuKyNguoiNhan");

                    b.Property<string>("ChuKyNguoiXuatKho")
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("ChuKyNguoiXuatKho");

                    b.Property<string>("ChuKyQuanLy")
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("ChuKyQuanLy");

                    b.Property<string>("GhiChu")
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("GhiChu");

                    b.Property<int?>("MaHoaDon")
                        .HasColumnType("int")
                        .HasColumnName("MaHoaDon");

                    b.Property<int?>("MaNguoiDuyet")
                        .HasColumnType("int")
                        .HasColumnName("MaNguoiDuyet");

                    b.Property<int?>("MaNguoiNhan")
                        .HasColumnType("int")
                        .HasColumnName("MaNguoiNhan");

                    b.Property<int?>("MaNguoiXuatKho")
                        .HasColumnType("int")
                        .HasColumnName("MaNguoiXuatKho");

                    b.Property<int?>("MaNhanVien")
                        .HasColumnType("int")
                        .HasColumnName("MaNhanVien");

                    b.Property<int?>("MaPhieuGH")
                        .HasColumnType("int")
                        .HasColumnName("MaPhieuGH");

                    b.Property<string>("MaXK")
                        .ValueGeneratedOnAddOrUpdate()
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("MaXK");

                    b.Property<DateTime?>("NgayDuyet")
                        .HasColumnType("datetime2")
                        .HasColumnName("NgayDuyet");

                    b.Property<DateTime>("NgayTao")
                        .HasColumnType("datetime2")
                        .HasColumnName("NgayTao");

                    b.Property<DateTime>("NgayXuat")
                        .HasColumnType("datetime2")
                        .HasColumnName("NgayXuat");

                    b.Property<string>("NguoiXuat")
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("NguoiXuat");

                    b.Property<string>("TrangThai")
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("TrangThai");

                    b.HasKey("MaPhieuXK");

                    b.HasIndex("MaHoaDon");

                    b.HasIndex("MaNguoiDuyet");

                    b.HasIndex("MaNhanVien");

                    b.HasIndex("MaPhieuGH");

                    b.ToTable("PHIEUXUATKHO");
                });

            modelBuilder.Entity("BuildingMaterialAPI.Models.Quyen", b =>
                {
                    b.Property<int>("MaQuyen")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasColumnName("MaQuyen");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("MaQuyen"));

                    b.Property<string>("MaQ")
                        .IsRequired()
                        .ValueGeneratedOnAddOrUpdate()
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("MaQ");

                    b.Property<string>("MoTa")
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("MoTa");

                    b.Property<DateTime>("NgayTao")
                        .HasColumnType("datetime2")
                        .HasColumnName("NgayTao");

                    b.Property<string>("TenQ")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("TenQ");

                    b.HasKey("MaQuyen");

                    b.ToTable("QUYEN");
                });

            modelBuilder.Entity("BuildingMaterialAPI.Models.SanPham", b =>
                {
                    b.Property<int>("MaSanPham")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasColumnName("MaSanPham");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("MaSanPham"));

                    b.Property<string>("AnhPhu")
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("AnhPhu");

                    b.Property<string>("DonViTinh")
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("DonViTinh");

                    b.Property<string>("DonViTrongLuong")
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("DonViTrongLuong");

                    b.Property<string>("GhiChu")
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("GhiChu");

                    b.Property<decimal>("GiaBan")
                        .HasColumnType("decimal(18,2)")
                        .HasColumnName("GiaBan");

                    b.Property<decimal?>("GiaNhap")
                        .HasColumnType("decimal(18,2)")
                        .HasColumnName("GiaNhap");

                    b.Property<string>("HinhAnh")
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("HinhAnh");

                    b.Property<bool>("IsGift")
                        .HasColumnType("bit")
                        .HasColumnName("IsGift");

                    b.Property<string>("KichThuoc")
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("KichThuoc");

                    b.Property<int>("MaLoaiSP")
                        .HasColumnType("int")
                        .HasColumnName("MaLoaiSP");

                    b.Property<string>("MaSP")
                        .IsRequired()
                        .ValueGeneratedOnAddOrUpdate()
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("MaSP");

                    b.Property<string>("MoTa")
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("MoTa");

                    b.Property<int>("MucTonToiThieu")
                        .HasColumnType("int")
                        .HasColumnName("MucTonToiThieu");

                    b.Property<DateTime>("NgayCapNhat")
                        .HasColumnType("datetime2")
                        .HasColumnName("NgayCapNhat");

                    b.Property<DateTime>("NgayTao")
                        .HasColumnType("datetime2")
                        .HasColumnName("NgayTao");

                    b.Property<string>("TenSP")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("TenSP");

                    b.Property<string>("ThuongHieu")
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("ThuongHieu");

                    b.Property<bool>("TrangThai")
                        .HasColumnType("bit")
                        .HasColumnName("TrangThai");

                    b.Property<decimal?>("TrongLuong")
                        .HasColumnType("decimal(18,2)")
                        .HasColumnName("TrongLuong");

                    b.Property<string>("XuatXu")
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("XuatXu");

                    b.HasKey("MaSanPham");

                    b.HasIndex("MaLoaiSP");

                    b.ToTable("SANPHAM");
                });

            modelBuilder.Entity("BuildingMaterialAPI.Models.TaiKhoan", b =>
                {
                    b.Property<int>("MaTaiKhoan")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasColumnName("MaTaiKhoan");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("MaTaiKhoan"));

                    b.Property<DateTime?>("DangNhapCuoi")
                        .HasColumnType("datetime2")
                        .HasColumnName("DangNhapCuoi");

                    b.Property<string>("Email")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("Email");

                    b.Property<string>("MaTK")
                        .IsRequired()
                        .ValueGeneratedOnAddOrUpdate()
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("MaTK");

                    b.Property<int>("MaVaiTro")
                        .HasColumnType("int")
                        .HasColumnName("MaVaiTro");

                    b.Property<string>("MatKhau")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("MatKhau");

                    b.Property<DateTime>("NgayCapNhat")
                        .HasColumnType("datetime2")
                        .HasColumnName("NgayCapNhat");

                    b.Property<DateTime>("NgayTao")
                        .HasColumnType("datetime2")
                        .HasColumnName("NgayTao");

                    b.Property<DateTime?>("OTPExpiry")
                        .HasColumnType("datetime2")
                        .HasColumnName("OTPExpiry");

                    b.Property<string>("ResetOTP")
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("ResetOTP");

                    b.Property<string>("TenTK")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("TenTK");

                    b.Property<bool>("TrangThai")
                        .HasColumnType("bit")
                        .HasColumnName("TrangThai");

                    b.HasKey("MaTaiKhoan");

                    b.HasIndex("MaVaiTro");

                    b.ToTable("TAIKHOAN");
                });

            modelBuilder.Entity("BuildingMaterialAPI.Models.ThongBao", b =>
                {
                    b.Property<int>("MaThongBao")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("MaThongBao"));

                    b.Property<bool>("DaDoc")
                        .HasColumnType("bit");

                    b.Property<string>("LienKet")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("LoaiThongBao")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("MaNguoiNhan")
                        .HasColumnType("nvarchar(max)");

                    b.Property<DateTime>("NgayTao")
                        .HasColumnType("datetime2");

                    b.Property<string>("NoiDung")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("TieuDe")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("MaThongBao");

                    b.ToTable("THONGBAO");
                });

            modelBuilder.Entity("BuildingMaterialAPI.Models.VaiTro", b =>
                {
                    b.Property<int>("MaVaiTro")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasColumnName("MaVaiTro");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("MaVaiTro"));

                    b.Property<string>("MaVT")
                        .IsRequired()
                        .ValueGeneratedOnAddOrUpdate()
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("MaVT");

                    b.Property<string>("MoTa")
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("MoTa");

                    b.Property<DateTime>("NgayCapNhat")
                        .HasColumnType("datetime2")
                        .HasColumnName("NgayCapNhat");

                    b.Property<DateTime>("NgayTao")
                        .HasColumnType("datetime2")
                        .HasColumnName("NgayTao");

                    b.Property<string>("TenVT")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("TenVT");

                    b.HasKey("MaVaiTro");

                    b.ToTable("VAITRO");
                });

            modelBuilder.Entity("BuildingMaterialAPI.Models.VoucherUuDai", b =>
                {
                    b.Property<int>("MaUUDAI")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasColumnName("MaUUDAI");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("MaUUDAI"));

                    b.Property<string>("Code")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)")
                        .HasColumnName("Code");

                    b.Property<decimal>("DonHangToiThieu")
                        .HasColumnType("decimal(18,2)")
                        .HasColumnName("DonHangToiThieu");

                    b.Property<decimal>("GiaTriGiam")
                        .HasColumnType("decimal(18,2)")
                        .HasColumnName("GiaTriGiam");

                    b.Property<decimal?>("GiamToiDa")
                        .HasColumnType("decimal(18,2)")
                        .HasColumnName("GiamToiDa");

                    b.Property<string>("HinhAnh")
                        .HasMaxLength(500)
                        .HasColumnType("nvarchar(500)")
                        .HasColumnName("HinhAnh");

                    b.Property<string>("LoaiUuDai")
                        .IsRequired()
                        .HasMaxLength(20)
                        .HasColumnType("nvarchar(20)")
                        .HasColumnName("LoaiUuDai");

                    b.Property<string>("MaVCDD")
                        .ValueGeneratedOnAddOrUpdate()
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("MaVCDD");

                    b.Property<string>("MoTa")
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("MoTa");

                    b.Property<DateTime>("NgayBatDau")
                        .HasColumnType("datetime2")
                        .HasColumnName("NgayBatDau");

                    b.Property<DateTime>("NgayCapNhat")
                        .HasColumnType("datetime2")
                        .HasColumnName("NgayCapNhat");

                    b.Property<DateTime>("NgayKetThuc")
                        .HasColumnType("datetime2")
                        .HasColumnName("NgayKetThuc");

                    b.Property<DateTime>("NgayTao")
                        .HasColumnType("datetime2")
                        .HasColumnName("NgayTao");

                    b.Property<int>("SoLuongDaDung")
                        .HasColumnType("int")
                        .HasColumnName("SoLuongDaDung");

                    b.Property<int?>("SoLuongToiDa")
                        .HasColumnType("int")
                        .HasColumnName("SoLuongToiDa");

                    b.Property<string>("TenUuDai")
                        .IsRequired()
                        .HasMaxLength(255)
                        .HasColumnType("nvarchar(255)")
                        .HasColumnName("TenUuDai");

                    b.Property<bool>("TrangThai")
                        .HasColumnType("bit")
                        .HasColumnName("TrangThai");

                    b.HasKey("MaUUDAI");

                    b.ToTable("UUDAI");
                });

            modelBuilder.Entity("BuildingMaterialAPI.Models.CTHD", b =>
                {
                    b.HasOne("BuildingMaterialAPI.Models.HoaDon", "HoaDon")
                        .WithMany("CTHDs")
                        .HasForeignKey("MaHoaDon")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("BuildingMaterialAPI.Models.SanPham", "SanPham")
                        .WithMany("CTHDs")
                        .HasForeignKey("MaSanPham")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("HoaDon");

                    b.Navigation("SanPham");
                });

            modelBuilder.Entity("BuildingMaterialAPI.Models.CTKhoHang", b =>
                {
                    b.HasOne("BuildingMaterialAPI.Models.KhoHang", "KhoHang")
                        .WithMany("CTKhoHangs")
                        .HasForeignKey("MaKhoHang")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("BuildingMaterialAPI.Models.SanPham", "SanPham")
                        .WithMany("CTKhoHangs")
                        .HasForeignKey("MaSanPham")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("KhoHang");

                    b.Navigation("SanPham");
                });

            modelBuilder.Entity("BuildingMaterialAPI.Models.CTPN", b =>
                {
                    b.HasOne("BuildingMaterialAPI.Models.KhoHang", "KhoHang")
                        .WithMany()
                        .HasForeignKey("MaKhoHang")
                        .OnDelete(DeleteBehavior.SetNull);

                    b.HasOne("BuildingMaterialAPI.Models.NhaCungCap", "NhaCungCap")
                        .WithMany()
                        .HasForeignKey("MaNhaCungCap");

                    b.HasOne("BuildingMaterialAPI.Models.PhieuNhap", "PhieuNhap")
                        .WithMany("CTPNs")
                        .HasForeignKey("MaPhieuNhap")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("BuildingMaterialAPI.Models.SanPham", "SanPham")
                        .WithMany("CTPNs")
                        .HasForeignKey("MaSanPham")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("KhoHang");

                    b.Navigation("NhaCungCap");

                    b.Navigation("PhieuNhap");

                    b.Navigation("SanPham");
                });

            modelBuilder.Entity("BuildingMaterialAPI.Models.CTPhieuDoiTra", b =>
                {
                    b.HasOne("BuildingMaterialAPI.Models.PhieuDoiTra", "PhieuDoiTra")
                        .WithMany("CTPhieuDoiTras")
                        .HasForeignKey("MaPhieuDT")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("BuildingMaterialAPI.Models.SanPham", "SanPham")
                        .WithMany("CTPhieuDoiTras")
                        .HasForeignKey("MaSanPham")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("PhieuDoiTra");

                    b.Navigation("SanPham");
                });

            modelBuilder.Entity("BuildingMaterialAPI.Models.CTPhieuGiaoHang", b =>
                {
                    b.HasOne("BuildingMaterialAPI.Models.PhieuGiaoHang", "PhieuGiaoHang")
                        .WithMany("CTPhieuGiaoHangs")
                        .HasForeignKey("MaPhieuGH")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("BuildingMaterialAPI.Models.SanPham", "SanPham")
                        .WithMany()
                        .HasForeignKey("MaSanPham")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.Navigation("PhieuGiaoHang");

                    b.Navigation("SanPham");
                });

            modelBuilder.Entity("BuildingMaterialAPI.Models.CTPhieuTraHangNCC", b =>
                {
                    b.HasOne("BuildingMaterialAPI.Models.PhieuTraHangNCC", "PhieuTraHangNCC")
                        .WithMany("ChiTiet")
                        .HasForeignKey("MaPhieuTra")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("BuildingMaterialAPI.Models.SanPham", "SanPham")
                        .WithMany()
                        .HasForeignKey("MaSanPham")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("PhieuTraHangNCC");

                    b.Navigation("SanPham");
                });

            modelBuilder.Entity("BuildingMaterialAPI.Models.CTPhieuXuatKho", b =>
                {
                    b.HasOne("BuildingMaterialAPI.Models.KhoHang", "KhoHang")
                        .WithMany()
                        .HasForeignKey("MaKho");

                    b.HasOne("BuildingMaterialAPI.Models.PhieuXuatKho", "PhieuXuatKho")
                        .WithMany("ChiTiet")
                        .HasForeignKey("MaPhieuXK")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("BuildingMaterialAPI.Models.SanPham", "SanPham")
                        .WithMany()
                        .HasForeignKey("MaSanPham")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("KhoHang");

                    b.Navigation("PhieuXuatKho");

                    b.Navigation("SanPham");
                });

            modelBuilder.Entity("BuildingMaterialAPI.Models.ChiTietTraNo", b =>
                {
                    b.HasOne("BuildingMaterialAPI.Models.CongNo", "CongNo")
                        .WithMany()
                        .HasForeignKey("MaCongNo")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.HasOne("BuildingMaterialAPI.Models.HoaDon", "HoaDon")
                        .WithMany("ChiTietTraNos")
                        .HasForeignKey("MaHoaDon")
                        .OnDelete(DeleteBehavior.Restrict);

                    b.HasOne("BuildingMaterialAPI.Models.NhanVien", "NhanVien")
                        .WithMany("ChiTietTraNos")
                        .HasForeignKey("MaNhanVien")
                        .OnDelete(DeleteBehavior.Restrict);

                    b.HasOne("BuildingMaterialAPI.Models.PhieuNhap", "PhieuNhap")
                        .WithMany()
                        .HasForeignKey("MaPhieuNhap")
                        .OnDelete(DeleteBehavior.Restrict);

                    b.Navigation("CongNo");

                    b.Navigation("HoaDon");

                    b.Navigation("NhanVien");

                    b.Navigation("PhieuNhap");
                });

            modelBuilder.Entity("BuildingMaterialAPI.Models.CongNo", b =>
                {
                    b.HasOne("BuildingMaterialAPI.Models.HoaDon", "HoaDon")
                        .WithMany("CongNos")
                        .HasForeignKey("MaHoaDon");

                    b.HasOne("BuildingMaterialAPI.Models.KhachHang", "KhachHang")
                        .WithMany("CongNos")
                        .HasForeignKey("MaKhachHang")
                        .OnDelete(DeleteBehavior.Restrict);

                    b.HasOne("BuildingMaterialAPI.Models.NhaCungCap", "NhaCungCap")
                        .WithMany("CongNos")
                        .HasForeignKey("MaNhaCungCap")
                        .OnDelete(DeleteBehavior.Restrict);

                    b.HasOne("BuildingMaterialAPI.Models.PhieuNhap", "PhieuNhap")
                        .WithMany("CongNos")
                        .HasForeignKey("MaPhieuNhap")
                        .OnDelete(DeleteBehavior.Restrict);

                    b.Navigation("HoaDon");

                    b.Navigation("KhachHang");

                    b.Navigation("NhaCungCap");

                    b.Navigation("PhieuNhap");
                });

            modelBuilder.Entity("BuildingMaterialAPI.Models.DanhGia", b =>
                {
                    b.HasOne("BuildingMaterialAPI.Models.HoaDon", "HoaDon")
                        .WithMany()
                        .HasForeignKey("MaHoaDon")
                        .OnDelete(DeleteBehavior.SetNull);

                    b.HasOne("BuildingMaterialAPI.Models.KhachHang", "KhachHang")
                        .WithMany("DanhGias")
                        .HasForeignKey("MaKhachHang")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.HasOne("BuildingMaterialAPI.Models.SanPham", "SanPham")
                        .WithMany("DanhGias")
                        .HasForeignKey("MaSanPham")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("HoaDon");

                    b.Navigation("KhachHang");

                    b.Navigation("SanPham");
                });

            modelBuilder.Entity("BuildingMaterialAPI.Models.HoaDon", b =>
                {
                    b.HasOne("BuildingMaterialAPI.Models.KhachHang", "KhachHang")
                        .WithMany("HoaDons")
                        .HasForeignKey("MaKhachHang")
                        .OnDelete(DeleteBehavior.Restrict);

                    b.HasOne("BuildingMaterialAPI.Models.KhuyenMai", "KhuyenMai")
                        .WithMany()
                        .HasForeignKey("MaKhuyenMai");

                    b.HasOne("BuildingMaterialAPI.Models.NhanVien", "NhanVien")
                        .WithMany("HoaDons")
                        .HasForeignKey("MaNhanVien")
                        .OnDelete(DeleteBehavior.Restrict);

                    b.Navigation("KhachHang");

                    b.Navigation("KhuyenMai");

                    b.Navigation("NhanVien");
                });

            modelBuilder.Entity("BuildingMaterialAPI.Models.KhachHang", b =>
                {
                    b.HasOne("BuildingMaterialAPI.Models.TaiKhoan", "TaiKhoan")
                        .WithOne("KhachHang")
                        .HasForeignKey("BuildingMaterialAPI.Models.KhachHang", "MaTaiKhoan")
                        .OnDelete(DeleteBehavior.SetNull);

                    b.Navigation("TaiKhoan");
                });

            modelBuilder.Entity("BuildingMaterialAPI.Models.KhuyenMaiDoiTuong", b =>
                {
                    b.HasOne("BuildingMaterialAPI.Models.KhuyenMai", "KhuyenMai")
                        .WithMany("KhuyenMaiDoiTuongs")
                        .HasForeignKey("MaKhuyenMai")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("BuildingMaterialAPI.Models.LoaiSanPham", "LoaiSanPham")
                        .WithMany("KhuyenMaiDoiTuongs")
                        .HasForeignKey("MaLoaiSP");

                    b.HasOne("BuildingMaterialAPI.Models.SanPham", "SanPham")
                        .WithMany("KhuyenMaiDoiTuongs")
                        .HasForeignKey("MaSanPham");

                    b.Navigation("KhuyenMai");

                    b.Navigation("LoaiSanPham");

                    b.Navigation("SanPham");
                });

            modelBuilder.Entity("BuildingMaterialAPI.Models.LichHenTraNo", b =>
                {
                    b.HasOne("BuildingMaterialAPI.Models.CongNo", "CongNo")
                        .WithMany()
                        .HasForeignKey("MaCongNo")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("CongNo");
                });

            modelBuilder.Entity("BuildingMaterialAPI.Models.LichSuGia", b =>
                {
                    b.HasOne("BuildingMaterialAPI.Models.NhanVien", "NhanVien")
                        .WithMany()
                        .HasForeignKey("MaNhanVien");

                    b.HasOne("BuildingMaterialAPI.Models.SanPham", "SanPham")
                        .WithMany()
                        .HasForeignKey("MaSanPham")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("NhanVien");

                    b.Navigation("SanPham");
                });

            modelBuilder.Entity("BuildingMaterialAPI.Models.LichSuGiaoHang", b =>
                {
                    b.HasOne("BuildingMaterialAPI.Models.PhieuGiaoHang", "PhieuGiaoHang")
                        .WithMany()
                        .HasForeignKey("MaPhieuGH")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("PhieuGiaoHang");
                });

            modelBuilder.Entity("BuildingMaterialAPI.Models.LichSuHoaDon", b =>
                {
                    b.HasOne("BuildingMaterialAPI.Models.HoaDon", "HoaDon")
                        .WithMany()
                        .HasForeignKey("MaHoaDon")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("BuildingMaterialAPI.Models.NhanVien", "NhanVien")
                        .WithMany()
                        .HasForeignKey("MaNguoiThucHien");

                    b.Navigation("HoaDon");

                    b.Navigation("NhanVien");
                });

            modelBuilder.Entity("BuildingMaterialAPI.Models.LichSuPhieuNhap", b =>
                {
                    b.HasOne("BuildingMaterialAPI.Models.NhanVien", "NhanVienThucHien")
                        .WithMany()
                        .HasForeignKey("MaNguoiThucHien");

                    b.HasOne("BuildingMaterialAPI.Models.PhieuNhap", "PhieuNhap")
                        .WithMany()
                        .HasForeignKey("MaPhieuNhap")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("NhanVienThucHien");

                    b.Navigation("PhieuNhap");
                });

            modelBuilder.Entity("BuildingMaterialAPI.Models.LichSuPhieuXuatKho", b =>
                {
                    b.HasOne("BuildingMaterialAPI.Models.NhanVien", "NhanVien")
                        .WithMany()
                        .HasForeignKey("MaNguoiThucHien");

                    b.HasOne("BuildingMaterialAPI.Models.PhieuXuatKho", "PhieuXuatKho")
                        .WithMany()
                        .HasForeignKey("MaPhieuXK")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("NhanVien");

                    b.Navigation("PhieuXuatKho");
                });

            modelBuilder.Entity("BuildingMaterialAPI.Models.LichSuThangHang", b =>
                {
                    b.HasOne("BuildingMaterialAPI.Models.KhachHang", "KhachHang")
                        .WithMany()
                        .HasForeignKey("MaKhachHang")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("KhachHang");
                });

            modelBuilder.Entity("BuildingMaterialAPI.Models.NhaCungCapSanPham", b =>
                {
                    b.HasOne("BuildingMaterialAPI.Models.NhaCungCap", "NhaCungCap")
                        .WithMany()
                        .HasForeignKey("MaNCC")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("BuildingMaterialAPI.Models.SanPham", "SanPham")
                        .WithMany("NhaCungCapSanPhams")
                        .HasForeignKey("MaSanPham")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("NhaCungCap");

                    b.Navigation("SanPham");
                });

            modelBuilder.Entity("BuildingMaterialAPI.Models.NhanVien", b =>
                {
                    b.HasOne("BuildingMaterialAPI.Models.TaiKhoan", "TaiKhoan")
                        .WithOne("NhanVien")
                        .HasForeignKey("BuildingMaterialAPI.Models.NhanVien", "MaTaiKhoan")
                        .OnDelete(DeleteBehavior.SetNull);

                    b.Navigation("TaiKhoan");
                });

            modelBuilder.Entity("BuildingMaterialAPI.Models.NhanVienModuleQuyen", b =>
                {
                    b.HasOne("BuildingMaterialAPI.Models.NhanVien", "NhanVien")
                        .WithMany()
                        .HasForeignKey("MaNhanVien")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("NhanVien");
                });

            modelBuilder.Entity("BuildingMaterialAPI.Models.NhatKy", b =>
                {
                    b.HasOne("BuildingMaterialAPI.Models.TaiKhoan", "TaiKhoan")
                        .WithMany("NhatKys")
                        .HasForeignKey("MaTaiKhoan")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.Navigation("TaiKhoan");
                });

            modelBuilder.Entity("BuildingMaterialAPI.Models.PhanQuyen", b =>
                {
                    b.HasOne("BuildingMaterialAPI.Models.Quyen", "Quyen")
                        .WithMany("PhanQuyens")
                        .HasForeignKey("MaQuyen")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("BuildingMaterialAPI.Models.VaiTro", "VaiTro")
                        .WithMany("PhanQuyens")
                        .HasForeignKey("MaVaiTro")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Quyen");

                    b.Navigation("VaiTro");
                });

            modelBuilder.Entity("BuildingMaterialAPI.Models.PhieuDoiTra", b =>
                {
                    b.HasOne("BuildingMaterialAPI.Models.HoaDon", "HoaDon")
                        .WithMany("PhieuDoiTras")
                        .HasForeignKey("MaHoaDon")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.HasOne("BuildingMaterialAPI.Models.NhanVien", "NhanVien")
                        .WithMany("PhieuDoiTras")
                        .HasForeignKey("MaNhanVien")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.Navigation("HoaDon");

                    b.Navigation("NhanVien");
                });

            modelBuilder.Entity("BuildingMaterialAPI.Models.PhieuGiaoHang", b =>
                {
                    b.HasOne("BuildingMaterialAPI.Models.HoaDon", "HoaDon")
                        .WithMany("PhieuGiaoHangs")
                        .HasForeignKey("MaHoaDon")
                        .OnDelete(DeleteBehavior.Restrict);

                    b.HasOne("BuildingMaterialAPI.Models.NhanVien", "NhanVien")
                        .WithMany("PhieuGiaoHangs")
                        .HasForeignKey("MaNhanVien")
                        .OnDelete(DeleteBehavior.Restrict);

                    b.Navigation("HoaDon");

                    b.Navigation("NhanVien");
                });

            modelBuilder.Entity("BuildingMaterialAPI.Models.PhieuNhap", b =>
                {
                    b.HasOne("BuildingMaterialAPI.Models.NhaCungCap", "NhaCungCap")
                        .WithMany("PhieuNhaps")
                        .HasForeignKey("MaNhaCungCap")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.HasOne("BuildingMaterialAPI.Models.NhanVien", "NhanVien")
                        .WithMany("PhieuNhaps")
                        .HasForeignKey("MaNhanVien")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.Navigation("NhaCungCap");

                    b.Navigation("NhanVien");
                });

            modelBuilder.Entity("BuildingMaterialAPI.Models.PhieuTraHangNCC", b =>
                {
                    b.HasOne("BuildingMaterialAPI.Models.NhanVien", "NhanVien")
                        .WithMany()
                        .HasForeignKey("MaNhanVien")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("BuildingMaterialAPI.Models.PhieuNhap", "PhieuNhap")
                        .WithMany()
                        .HasForeignKey("MaPhieuNhap")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("NhanVien");

                    b.Navigation("PhieuNhap");
                });

            modelBuilder.Entity("BuildingMaterialAPI.Models.PhieuXuatKho", b =>
                {
                    b.HasOne("BuildingMaterialAPI.Models.HoaDon", "HoaDon")
                        .WithMany()
                        .HasForeignKey("MaHoaDon");

                    b.HasOne("BuildingMaterialAPI.Models.NhanVien", "NguoiDuyet")
                        .WithMany()
                        .HasForeignKey("MaNguoiDuyet");

                    b.HasOne("BuildingMaterialAPI.Models.NhanVien", "NhanVien")
                        .WithMany()
                        .HasForeignKey("MaNhanVien");

                    b.HasOne("BuildingMaterialAPI.Models.PhieuGiaoHang", "PhieuGiaoHang")
                        .WithMany()
                        .HasForeignKey("MaPhieuGH");

                    b.Navigation("HoaDon");

                    b.Navigation("NguoiDuyet");

                    b.Navigation("NhanVien");

                    b.Navigation("PhieuGiaoHang");
                });

            modelBuilder.Entity("BuildingMaterialAPI.Models.SanPham", b =>
                {
                    b.HasOne("BuildingMaterialAPI.Models.LoaiSanPham", "LoaiSanPham")
                        .WithMany("SanPhams")
                        .HasForeignKey("MaLoaiSP")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.Navigation("LoaiSanPham");
                });

            modelBuilder.Entity("BuildingMaterialAPI.Models.TaiKhoan", b =>
                {
                    b.HasOne("BuildingMaterialAPI.Models.VaiTro", "VaiTro")
                        .WithMany("TaiKhoans")
                        .HasForeignKey("MaVaiTro")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.Navigation("VaiTro");
                });

            modelBuilder.Entity("BuildingMaterialAPI.Models.HoaDon", b =>
                {
                    b.Navigation("CTHDs");

                    b.Navigation("ChiTietTraNos");

                    b.Navigation("CongNos");

                    b.Navigation("PhieuDoiTras");

                    b.Navigation("PhieuGiaoHangs");
                });

            modelBuilder.Entity("BuildingMaterialAPI.Models.KhachHang", b =>
                {
                    b.Navigation("CongNos");

                    b.Navigation("DanhGias");

                    b.Navigation("HoaDons");
                });

            modelBuilder.Entity("BuildingMaterialAPI.Models.KhoHang", b =>
                {
                    b.Navigation("CTKhoHangs");
                });

            modelBuilder.Entity("BuildingMaterialAPI.Models.KhuyenMai", b =>
                {
                    b.Navigation("KhuyenMaiDoiTuongs");
                });

            modelBuilder.Entity("BuildingMaterialAPI.Models.LoaiSanPham", b =>
                {
                    b.Navigation("KhuyenMaiDoiTuongs");

                    b.Navigation("SanPhams");
                });

            modelBuilder.Entity("BuildingMaterialAPI.Models.NhaCungCap", b =>
                {
                    b.Navigation("CongNos");

                    b.Navigation("PhieuNhaps");
                });

            modelBuilder.Entity("BuildingMaterialAPI.Models.NhanVien", b =>
                {
                    b.Navigation("ChiTietTraNos");

                    b.Navigation("HoaDons");

                    b.Navigation("PhieuDoiTras");

                    b.Navigation("PhieuGiaoHangs");

                    b.Navigation("PhieuNhaps");
                });

            modelBuilder.Entity("BuildingMaterialAPI.Models.PhieuDoiTra", b =>
                {
                    b.Navigation("CTPhieuDoiTras");
                });

            modelBuilder.Entity("BuildingMaterialAPI.Models.PhieuGiaoHang", b =>
                {
                    b.Navigation("CTPhieuGiaoHangs");
                });

            modelBuilder.Entity("BuildingMaterialAPI.Models.PhieuNhap", b =>
                {
                    b.Navigation("CTPNs");

                    b.Navigation("CongNos");
                });

            modelBuilder.Entity("BuildingMaterialAPI.Models.PhieuTraHangNCC", b =>
                {
                    b.Navigation("ChiTiet");
                });

            modelBuilder.Entity("BuildingMaterialAPI.Models.PhieuXuatKho", b =>
                {
                    b.Navigation("ChiTiet");
                });

            modelBuilder.Entity("BuildingMaterialAPI.Models.Quyen", b =>
                {
                    b.Navigation("PhanQuyens");
                });

            modelBuilder.Entity("BuildingMaterialAPI.Models.SanPham", b =>
                {
                    b.Navigation("CTHDs");

                    b.Navigation("CTKhoHangs");

                    b.Navigation("CTPNs");

                    b.Navigation("CTPhieuDoiTras");

                    b.Navigation("DanhGias");

                    b.Navigation("KhuyenMaiDoiTuongs");

                    b.Navigation("NhaCungCapSanPhams");
                });

            modelBuilder.Entity("BuildingMaterialAPI.Models.TaiKhoan", b =>
                {
                    b.Navigation("KhachHang")
                        .IsRequired();

                    b.Navigation("NhanVien")
                        .IsRequired();

                    b.Navigation("NhatKys");
                });

            modelBuilder.Entity("BuildingMaterialAPI.Models.VaiTro", b =>
                {
                    b.Navigation("PhanQuyens");

                    b.Navigation("TaiKhoans");
                });
#pragma warning restore 612, 618
        }
    }
}
